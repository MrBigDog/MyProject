//已经分好块 分好级的数据，比如 倾斜摄影 点云类型的数据;
//这类数据一般顶点数量比较多，所以用VBO的方式绘制是个很好的选择;
#ifndef GWDRIVERS_TILEDMODELOPTIONS_H
#define GWDRIVERS_TILEDMODELOPTIONS_H 1
...
namespace gwDrivers
{
	class TiledModelOptions : public ModelSourceOptions // NO EXPORT; header only
	{
	public:
	...
	public:
		TiledModelOptions(const ConfigOptions& options = ConfigOptions())
	...
	public:
		Config getConfig() const {
			...
			conf.updateNonSerializable("TiledModelOptions::Node", _node.get());
			return conf;
		}
	...
	private:
		void fromConfig(const Config& conf)
		{
			...
			_node = conf.getNonSerializable<osg::Node>("TiledModelOptions::Node");
		}
...
#endif // GWDRIVERS_TILEDMODELOPTIONS_H
