...
///////////////////////////////////////////////////////////////////////////
//
//  Filename: us_interior_index_factory.cpp
...
namespace uniscope_globe
{
	...
	ustring interior_index_factory::to_encrypt_string(interior_index index)
	{
		...
		sprintf(str_temp, "%x/%x%x/%x%x/%I64x__", level, lon1, lat1, lon2, lat2, index);
		str_path = str_temp;
#endif //_UNICODE
	...
	ustring interior_index_factory::to_wmts_string_by_lonlat(interior_index index, bool xylformat)
	{
		int level = INTERIOR_INDEX_LEVEL(index);
		long lon_index = INTERIOR_INDEX_LON(index);
		long lat_index = INTERIOR_INDEX_LAT(index);
		...
		int n = 2 ^ level;
		int xtile = n * ((lon1* math_d::s_rad_to_deg + 180) / 360);
		int ytile = n * (1 - (log(tan(lat1* math_d::s_rad_to_deg) + (1 / cos(lat1* math_d::s_rad_to_deg))) / PI)) / 2;
		...
		wchar_t str_temp[MAX_FILE_PATH];
		if (xylformat)
			swprintf(str_temp, MAX_FILE_PATH, L"x=%d&y=%d&z=%d", xtile, ytile, level);
		else
			swprintf(str_temp, MAX_FILE_PATH, L"TileMatrix=%d&TileRow=%d&TileCol=%d", level, xtile, ytile);
		str_path = str_temp;
#else
		char str_temp[MAX_FILE_PATH];
		//sprintf( str_temp, "TileMatrix=%d&TileRow=%d&TileCol=%d", level, lat_index, lon_index );
		if (xylformat)
			//swprintf(str_temp, MAX_FILE_PATH, L"x=%d&y=%d&l=%d", xtile, ytile, level);
			sprintf(str_temp, /*MAX_FILE_PATH, L*/"x=%d&y=%d&l=%d", xtile, ytile, level);
		else
			//swprintf(str_temp, MAX_FILE_PATH, L"TileMatrix=%d&TileRow=%d&TileCol=%d", level, xtile, ytile);
			sprintf(str_temp, /*MAX_FILE_PATH, L*/"TileMatrix=%d&TileRow=%d&TileCol=%d", level, xtile, ytile);
		str_path = str_temp;
#endif //_UNICODE
	...
	ustring interior_index_factory::to_wmts_string(interior_index index, bool xylformat)
	{
		int level = INTERIOR_INDEX_LEVEL(index);
		long lon_index = INTERIOR_INDEX_LON(index);
		long lat_index = INTERIOR_INDEX_LAT(index);
		...
		/*int  north_index,  south_index, east_index,  west_index ;
		get_int_rect(get_bound(index), level, north_index,  south_index, east_index,  west_index );*/
		/*lon_index = east_index;
		lat_index = north_index;*/
		...
		wchar_t str_temp[MAX_FILE_PATH];
		if (xylformat)
			swprintf(str_temp, MAX_FILE_PATH, L"x=%d&y=%d&z=%d", lon_index, lat_index, level);
		else
			swprintf(str_temp, MAX_FILE_PATH, L"TileMatrix=%d&TileRow=%d&TileCol=%d", level, lon_index, lat_index);
		str_path = str_temp;
#else
		char str_temp[MAX_FILE_PATH];
		//sprintf( str_temp, "TileMatrix=%d&TileRow=%d&TileCol=%d", level, lat_index, lon_index );
		if (xylformat)
			//swprintf(str_temp, MAX_FILE_PATH, L"x=%d&y=%d&l=%d", lat_index, lon_index, level);
			sprintf(str_temp, /*MAX_FILE_PATH,*/ /*L*/"x=%d&y=%d&l=%d", lat_index, lon_index, level);
		else
			//swprintf(str_temp, MAX_FILE_PATH, L"TileMatrix=%d&TileRow=%d&TileCol=%d", level, lat_index, lon_index);
			sprintf(str_temp, /*MAX_FILE_PATH, L*/"TileMatrix=%d&TileRow=%d&TileCol=%d", level, lat_index, lon_index);
		str_path = str_temp;
#endif //_UNICODE
	...
	void interior_index_factory::get_int_rect( /*IN*/ lon_lat_rect& rect, /*IN*/ int level, /*OUT*/ int& north_index, /*OUT*/ int& south_index, /*OUT*/ int& east_index, /*OUT*/ int& west_index)
	{
		double v_bound_size_reciprocal = 1.0 / get_bound_size(level);
	...
	void interior_index_helper::get_int_rect( /*IN*/ lon_lat_rect& rect, /*IN*/ int level, /*OUT*/ int& north_index, /*OUT*/ int& south_index, /*OUT*/ int& east_index, /*OUT*/ int& west_index)
	{
		double v_bound_size_reciprocal = 1.0 / get_bound_size(level);
		west_index = int(floor((rect.m_west - m_world_rect.m_west) * v_bound_size_reciprocal));
		south_index = int(floor((rect.m_south - m_world_rect.m_south) * v_bound_size_reciprocal));
