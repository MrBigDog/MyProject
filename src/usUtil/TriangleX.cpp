...
/*****************************************************************************/
/*  This define for MSVC DLL complier                                        */
/*****************************************************************************/
#define NO_TIMER
#define REDUCED
#define CDT_ONLY
...
#undef	REAL
#undef TRI_USE_SINGLE
#define TRI_USE_SINGLE
#ifdef	TRI_USE_SINGLE
#define REAL float
#else /* not SINGLE */
#define REAL double
#endif /* not SINGLE */
...
#define INPUTLINESIZE _MAX_PATH
...
/* exit() may or may not already be defined at this myPoint.  I declare these  */
...
/*   floating-myPoint words.  Used to make decisions about data alignment.     */
...
typedef REAL** myTriangle;            /* Really:  typedef myTriangle *myTriangle   */
...
typedef REAL* myPoint;


struct badsegment
{
	struct edge encsegment;                          /* An encroached segment. */
	myPoint segorg, segdest;                                /* The two vertices. */
...
struct badface
{
	struct triedge badfacetri;                              /* A bad myTriangle. */
	REAL key;                             /* cos^2 of smallest (apical) angle. */
	myPoint faceorg, facedest, faceapex;                  /* The three vertices. */
	struct badface* nextface;                 /* Pointer to next bad myTriangle. */
};
...
struct event
{
	...
	VOID* eventptr;       /* Can be a myPoint or the location of a circle event. */
...
struct splaynode
{
	...
	myPoint keydest;            /* Used to verify that splay node is still live. */
...
/* Variables that maintain the bad myTriangle queues.  The tails are pointers  */
...
int nextras;                              /* Number of attributes per myPoint. */
int eextras;                           /* Number of attributes per myTriangle. */
long hullsize;                            /* Number of edges of convex hull. */
int triwords;                                   /* Total words per myTriangle. */
int shwords;                                  /* Total words per shell edge. */
int pointmarkindex;             /* Index to find boundary marker of a myPoint. */
int point2triindex;         /* Index to find a myTriangle adjacent to a myPoint. */
int highorderindex;    /* Index to find extra nodes for high-order elements. */
int elemattribindex;              /* Index to find attributes of a myTriangle. */
int areaboundindex;               /* Index to find area bound of a myTriangle. */
int checksegments;           /* Are there segments in the triangulation yet? */
int readnodefile;                             /* Has a .node file been read? */
long samples;                /* Number of random samples for myPoint location. */
unsigned long randomseed;                     /* Current random number seed. */

REAL splitter;       /* Used to split REAL factors for exact multiplication. */
REAL epsilon;                             /* Floating-myPoint machine epsilon. */
REAL resulterrbound;
...
//记录edcode的地址指针，临时使用
// CMap <myPoint,myPoint,myPoint,myPoint>	g_pTrMap;
std::map <myPoint, myPoint>	g_pTrMap;
...
myPoint infpoint1, infpoint2, infpoint3;

/* Pointer to the `myTriangle' that occupies all of "outer space".             */

myTriangle* dummytri;
myTriangle* dummytribase;      /* Keep base address so we can free() it later. */

/* Pointer to the omnipresent shell edge.  Referenced by any myTriangle or     */
...
/* Pointer to a recently visited myTriangle.  Improves myPoint location if       */
...
#define decode(ptr, triedge)                                                  \
  (triedge).orient = (int) ((ULONG_PTR) (ptr) & (unsigned long) 3l);      \
  (triedge).tri = (myTriangle *)                                                \
                  ((ULONG_PTR) (ptr) ^ (unsigned long) (triedge).orient)

/* encode() compresses an oriented myTriangle into a single pointer.  It       */
/*   relies on the assumption that all triangles are aligned to four-byte    */
/*   boundaries, so the two least significant bits of (triedge).tri are zero.*/

#define encode(triedge)                                                       \
  (myTriangle) ((ULONG_PTR) (triedge).tri | (unsigned long) (triedge).orient)
...
/* lnext() finds the next edge (counterclockwise) of a myTriangle.             */
...
/* lprev() finds the previous edge (clockwise) of a myTriangle.                */
...
/* onext() spins counterclockwise around a myPoint; that is, it finds the next */
/*   edge with the same origin in the counterclockwise direction.  This edge */
/*   will be part of a different myTriangle.                                   */
...
/* oprev() spins clockwise around a myPoint; that is, it finds the next edge   */
/*   with the same origin in the clockwise direction.  This edge will be     */
/*   part of a different myTriangle.                                           */
...
/* dnext() spins counterclockwise around a myPoint; that is, it finds the next */
/*   edge with the same destination in the counterclockwise direction.  This */
/*   edge will be part of a different myTriangle.                              */
...
/* dprev() spins clockwise around a myPoint; that is, it finds the next edge   */
/*   with the same destination in the clockwise direction.  This edge will   */
/*   be part of a different myTriangle.                                        */
...
/* rnext() moves one edge counterclockwise about the adjacent myTriangle.      */
...
/* rnext() moves one edge clockwise about the adjacent myTriangle.             */
/*   (It's best understood by reading Guibas and Stolfi.  It involves        */
/*   changing triangles twice.)                                              */
...
#define org(triedge, pointptr)                                                \
  pointptr = (myPoint) (triedge).tri[plus1mod3[(triedge).orient] + 3]

#define dest(triedge, pointptr)                                               \
  pointptr = (myPoint) (triedge).tri[minus1mod3[(triedge).orient] + 3]

#define apex(triedge, pointptr)                                               \
  pointptr = (myPoint) (triedge).tri[(triedge).orient + 3]

#define setorg(triedge, pointptr)                                             \
  (triedge).tri[plus1mod3[(triedge).orient] + 3] = (myTriangle) pointptr

#define setdest(triedge, pointptr)                                            \
  (triedge).tri[minus1mod3[(triedge).orient] + 3] = (myTriangle) pointptr

#define setapex(triedge, pointptr)                                            \
  (triedge).tri[(triedge).orient + 3] = (myTriangle) pointptr

#define setvertices2null(triedge)                                             \
  (triedge).tri[3] = (myTriangle) NULL;                                         \
  (triedge).tri[4] = (myTriangle) NULL;                                         \
  (triedge).tri[5] = (myTriangle) NULL;
...
/* Dissolve a bond (from one side).  Note that the other myTriangle will still */
/*   think it's connected to this myTriangle.  Usually, however, the other     */
/*   myTriangle is being deleted entirely, or bonded to another myTriangle, so   */
/*   it doesn't matter.                                                      */

#define dissolve(triedge)                                                     \
  (triedge).tri[(triedge).orient] = (myTriangle) dummytri

/* Copy a myTriangle/edge handle.                                              */

#define triedgecopy(triedge1, triedge2)                                       \
  (triedge2).tri = (triedge1).tri;                                            \
  (triedge2).orient = (triedge1).orient

/* Test for equality of myTriangle/edge handles.                               */

#define triedgeequal(triedge1, triedge2)                                      \
  (((triedge1).tri == (triedge2).tri) &&                                      \
   ((triedge1).orient == (triedge2).orient))

/* Primitives to infect or cure a myTriangle with the virus.  These rely on    */
/*   the assumption that all shell edges are aligned to four-byte boundaries.*/

#define infect(triedge)                                                       \
  (triedge).tri[6] = (myTriangle)                                               \
                     ((ULONG_PTR) (triedge).tri[6] | (unsigned long) 2l)

#define uninfect(triedge)                                                     \
  (triedge).tri[6] = (myTriangle)                                               \
                     ((ULONG_PTR) (triedge).tri[6] & ~ (unsigned long) 2l)

/* Test a myTriangle for viral infection.                                      */

#define infected(triedge)                                                     \
  (((ULONG_PTR) (triedge).tri[6] & (unsigned long) 2l) != 0)

/* Check or set a myTriangle's attributes.                                     */
...
/* Check or set a myTriangle's maximum area bound.                             */
...
#define sdecode(sptr, edge)                                                   \
  (edge).shorient = (int) ((ULONG_PTR) (sptr) & (unsigned long) 1l);      \
  (edge).sh = (shelle *)                                                      \
              ((ULONG_PTR) (sptr) & ~ (unsigned long) 3l)
...
#define sencode(edge)                                                         \
  (shelle) ((ULONG_PTR) (edge).sh | (unsigned long) (edge).shorient)
...
#define sorg(edge, pointptr)                                                  \
  pointptr = (myPoint) (edge).sh[2 + (edge).shorient]

#define sdest(edge, pointptr)                                                 \
  pointptr = (myPoint) (edge).sh[3 - (edge).shorient]
...
/* tspivot() finds a shell edge abutting a myTriangle.                         */

#define tspivot(triedge, edge)                                                \
  sptr = (shelle) (triedge).tri[6 + (triedge).orient];                        \
  sdecode(sptr, edge)

/* stpivot() finds a myTriangle abutting a shell edge.  It requires that the   */
/*   variable `ptr' of type `myTriangle' be defined.                           */

#define stpivot(edge, triedge)                                                \
  ptr = (myTriangle) (edge).sh[4 + (edge).shorient];                            \
  decode(ptr, triedge)

/* Bond a myTriangle to a shell edge.                                          */

#define tsbond(triedge, edge)                                                 \
  (triedge).tri[6 + (triedge).orient] = (myTriangle) sencode(edge);             \
  (edge).sh[4 + (edge).shorient] = (shelle) encode(triedge)

/* Dissolve a bond (from the myTriangle side).                                 */

#define tsdissolve(triedge)                                                   \
  (triedge).tri[6 + (triedge).orient] = (myTriangle) dummysh
...
#define point2tri(pt)  ((myTriangle *) (pt))[point2triindex]

#define setpoint2tri(pt, value)                                               \
  ((myTriangle *) (pt))[point2triindex] = value
...
void syntax()
{
#ifdef CDT_ONLY
#ifdef REDUCED
	printf("myTriangle [-pAcevngBPNEIOXzo_lQVh] input_file\n");
#else /* not REDUCED */
	printf("myTriangle [-pAcevngBPNEIOXzo_iFlCQVh] input_file\n");
#endif /* not REDUCED */
#else /* not CDT_ONLY */
#ifdef REDUCED
	printf("myTriangle [-prq__a__AcevngBPNEIOXzo_YS__lQVh] input_file\n");
#else /* not REDUCED */
	printf("myTriangle [-prq__a__AcevngBPNEIOXzo_YS__iFlsCQVh] input_file\n");
#endif /* not REDUCED */
#endif /* not CDT_ONLY */
	...
	printf("    -a  Applies a maximum myTriangle area constraint.\n");
#endif /* not CDT_ONLY */
	printf(
	...
	printf("    -n  Generates a list of myTriangle neighbors.\n");
...
void info()
{
	...
	printf(
		"        with insufficient floating-myPoint precision.  The specified angle\n");
	printf("        may include a decimal myPoint.\n");
	printf(
		"    -a  Imposes a maximum myTriangle area.  If a number follows the `a', no\n");
	printf(
		"        myTriangle will be generated whose area is larger than that number.\n");
	printf(
	...
	printf(
		"        each myTriangle, and is useful for refining a finite element mesh\n");
	printf(
	...
	printf(
		"        enforcing myTriangle densities in a first triangulation.  You can\n");
	printf(
	...
	printf(
		"        number following.  Each area specified may include a decimal myPoint.\n"
	);
	printf(
		"    -A  Assigns an additional attribute to each myTriangle that identifies\n");
	printf(
		"        what segment-bounded region each myTriangle belongs to.  Attributes\n");
	printf(
	...
	printf(
		"        are triangulating a myPoint set, this switch causes a .poly file to\n");
	printf(
	...
	printf(
		"    -X  No exact arithmetic.  Normally, Triangle uses exact floating-myPoint\n"
	);
	printf(
	...
	printf(
		"        triangulation algorithms, despite floating-myPoint roundoff error.\n");
	printf(
	...
	printf(
		"        first myPoint of the input .node or .poly file.  However, this switch\n"
	);
	...
	printf(
		"        which usually improve the speed except with myPoint sets that are\n");
	printf(
	...
	printf(
		"        `-VV' gives myPoint-by-myPoint details, and will print so much that\n");
	printf(
	...
	printf("\n");
	printf(
		"  A Delaunay triangulation of a myPoint set is a triangulation whose vertices\n"
	);
	printf(
		"  are the myPoint set, having the property that no myPoint in the myPoint set\n");
	printf(
		"  falls in the interior of the circumcircle (circle that passes through all\n"
	);
	printf("  three vertices) of any myTriangle in the triangulation.\n\n");
	printf(
		"  A Voronoi diagram of a myPoint set is a subdivision of the plane into\n");
	printf(
		"  polygonal regions (some of which may be infinite), where each region is\n");
	printf(
		"  the set of points in the plane that are closer to some input myPoint than\n");
	printf(
		"  to any other input myPoint.  (The Voronoi diagram is the geometric dual of\n"
	);
	...
	printf(
		"    Remaining lines:  <myPoint #> <x> <y> [attributes] [boundary marker]\n");
	printf("\n");
	printf(
		"    The attributes, which are typically floating-myPoint values of physical\n");
	printf(
	...
	printf(
		"    -q, or -a is selected, each new Steiner myPoint added to the mesh will\n");
	...
	printf("  .ele files:\n");
	printf(
		"    First line:  <# of triangles> <points per myTriangle> <# of attributes>\n");
	printf(
		"    Remaining lines:  <myTriangle #> <myPoint> <myPoint> <myPoint> ... [attributes]\n"
	);
	printf("\n");
	printf(
	...
	printf(
		"    each myTriangle.  (The remaining points, if any, depend on the type of\n");
	printf(
	...
	printf(
		"    Following lines:  <myPoint #> <x> <y> [attributes] [boundary marker]\n");
	...
	printf(
		"    Triangle always have this format.  This has the advantage that a myPoint\n"
	);
	printf(
	...
	printf(
		"    include its endpoints in the myPoint list.  If -s, -q, and -a are not\n");
	printf(
	...
	printf("    Each segment, like each myPoint, may have a boundary marker.\n\n");
	printf(
		"    The third section lists holes (and concavities, if -c is selected) in\n");
	printf(
		"    the triangulation.  Holes are specified by identifying a myPoint inside\n");
	printf(
		"    each hole.  After the triangulation is formed, Triangle creates holes\n");
	printf(
		"    by eating triangles, spreading out from each hole myPoint until its\n");
	printf(
	...
	printf(
		"    myTriangle area.  Triangle will read this section only if the -A switch\n");
	printf(
	...
	printf(
		"    propagated in the same manner as holes; you specify a myPoint for each\n");
	printf(
		"    attribute and/or constraint, and the attribute and/or constraint will\n");
	printf(
		"    affect the whole region (bounded by segments) containing the myPoint.  If\n"
	);
	printf(
	...
	printf(
		"    use the -c switch, which encloses the convex hull of the input myPoint\n");
	printf(
	...
	printf(
		"    the triangulation - as long as your machine's floating-myPoint precision\n"
	);
	printf(
	...
	printf(
		"    out where the intersection myPoint is.  Thanks to floating-myPoint roundoff\n"
	);
	printf(
		"    error, Triangle will probably decide that the three segments intersect\n"
	);
	printf(
		"    at three different points, and you will find a minuscule myTriangle in\n");
	printf(
		"    your output - unless Triangle tries to refine the tiny myTriangle, uses\n");
	printf(
		"    up the last bit of machine precision, and fails to terminate at all.\n");
	printf(
		"    You're better off putting the intersection myPoint in the input files,\n");
	printf(
		"    and manually breaking up each segment into two.  Similarly, if you\n");
	printf(
		"    place a myPoint at the middle of a segment, and hope that Triangle will\n");
	printf(
		"    break up the segment at that myPoint, you might get lucky.  On the other\n"
	);
	printf(
		"    hand, Triangle might decide that the myPoint doesn't lie precisely on the\n"
	);
	printf(
		"    line, and you'll have a needle-sharp myTriangle in your output - or a lot\n"
	);
	...
	printf("    Following lines:  <myTriangle #> <maximum area>\n\n");
	printf(
		"    An .area file associates with each myTriangle a maximum area that is used\n"
	);
	printf(
		"    for mesh refinement.  As with other file formats, every myTriangle must\n");
	printf(
		"    be represented, and they must be numbered consecutively.  A myTriangle\n");
	printf(
	...
	printf(
		"    The `direction' is a floating-myPoint vector that indicates the direction\n"
	);
	printf("    of the infinite ray.\n\n");
	printf("  .neigh files:\n");
	printf(
		"    First line:  <# of triangles> <# of neighbors per myTriangle (always 3)>\n"
	);
	printf(
		"    Following lines:  <myTriangle #> <neighbor> <neighbor> <neighbor>\n");
	printf("\n");
	printf(
	...
	printf("\n");
	printf(
		"    The first neighbor of myTriangle i is opposite the first corner of\n");
	printf("    myTriangle i, and so on.\n\n");
	...
	printf(
		"  The boundary marker associated with each myPoint in an output .node file is\n"
	);
	printf("  chosen as follows:\n");
	printf(
		"    - If a myPoint is assigned a nonzero boundary marker in the input file,\n");
	printf(
		"      then it is assigned the same marker in the output .node file.\n");
	printf(
		"    - Otherwise, if the myPoint lies on a PSLG segment (including the\n");
	printf(
		"      segment's endpoints) with a nonzero boundary marker, then the myPoint\n");
	printf(
		"      is assigned the same marker.  If the myPoint lies on several such\n");
	printf("      segments, one of the markers is chosen arbitrarily.\n");
	printf(
		"    - Otherwise, if the myPoint occurs on a boundary of the triangulation,\n");
	printf("      then the myPoint is assigned the marker one (1).\n");
	printf("    - Otherwise, the myPoint is assigned the marker zero (0).\n");
	printf("\n");
	printf(
	...
	printf(
		"  `myTriangle dots' will read points from dots.node, and write their Delaunay\n"
	);
	printf(
		"  triangulation to dots.1.node and dots.1.ele.  (dots.1.node will be\n");
	printf(
		"  identical to dots.node.)  `myTriangle -I dots' writes the triangulation to\n"
	);
	printf(
		"  dots.ele instead.  (No additional .node file is needed, so none is\n");
	printf("  written.)\n\n");
	printf(
		"  `myTriangle -pe object.1' will read a PSLG from object.1.poly (and possibly\n"
	);
	printf(
	...
	printf(
		"  `myTriangle -pq31.5a.1 object' will read a PSLG from object.poly (and\n");
	printf(
	...
	printf("     2   0 3   0\n");
	printf("     3   3 0   0\n");
	printf("     4   3 3   33     # A special marker for this myPoint.\n");
	...
	printf(
		"  use the `-c' switch.  After `myTriangle -pqc box.poly', here is the output\n"
	);
	printf(
	...
	printf("      11    3   1.5    1\n");
	printf("      12    1.5   3    1\n");
	printf("    # Generated by myTriangle -pqc box.poly\n\n");
	...
	printf(
		"  file, which specifies a maximum area for each myTriangle, and use the -a\n");
	printf(
		"  switch (without a number following).  Each myTriangle's area constraint is\n"
	);
	printf(
		"  applied to that myTriangle.  Area constraints tend to diffuse as the mesh\n");
	printf(
	...
	printf(
		"  myPoint falls, whereas a maximum area in an .area file applies to only one\n"
	);
	printf(
		"  myTriangle.  Area constraints in .poly files are used only when a mesh is\n");
	printf(
	...
	printf("\n");
	printf(
		"  `myTriangle -rq25 object.1' will read object.1.node and object.1.ele, then\n"
	);
	printf(
	...
	printf("\n");
	printf(
		"  `myTriangle -rpaa6.2 z.3' will read z.3.node, z.3.ele, z.3.poly, and\n");
	printf(
		"  z.3.area.  After reconstructing the mesh and its segments, Triangle will\n"
	);
	printf(
		"  refine the mesh so that no myTriangle has area greater than 6.2, and\n");
	printf(
	...
	printf("\n");
	printf(
		"  The sequence `myTriangle -qa1 x', `myTriangle -rqa.3 x.1', `myTriangle -rqa.1\n");
	printf(
		"  x.2' creates a sequence of successively finer meshes x.1, x.2, and x.3,\n");
	printf("  suitable for multigrid.\n\n");
	printf("Convex Hulls and Mesh Boundaries:\n\n");
	printf(
		"  If the input is a myPoint set (rather than a PSLG), Triangle produces its\n");
	printf(
	...
	printf(
		"  .v.edge.  For example, `myTriangle -v points' will read points.node,\n");
	printf(
	...
	printf("  dual of myPoint k of the corresponding .node file.\n\n");
	printf(
	...
	printf(
		"  of memory use and a histogram of myTriangle aspect ratios and angles in the\n"
	);
	printf("  mesh.\n\n");
	...
	printf(
		"  myPoint arithmetic of your machine conforms to the IEEE 754 standard (as\n");
	printf(
	...
	printf(
		"  bit complexity of myPoint coordinates would grow without bound.  Hence,\n");
	printf(
		"  segment intersections aren't computed exactly; in very unusual cases,\n");
	printf(
		"  roundoff error in computing an intersection myPoint might actually lead to\n"
	);
	printf(
		"  an inverted myTriangle and an invalid triangulation.  (This is one reason\n");
	printf(
	...
	printf("  Read the file myTriangle.h for details.\n\n");
	...
	printf(
		"    inside your myPoint set, or even inside the densest part of your\n");
	printf(
	...
	printf("    much floating-myPoint precision for Triangle to work with.\n\n");
	printf(
	...
	printf(
		"    angle is introduced by the -c switch, which may occur if a myPoint lies\n");
	printf(
		"    ever-so-slightly inside the convex hull, and is connected by a PSLG\n");
	printf(
		"    segment to a myPoint on the convex hull.  If you don't realize that a\n");
	printf(
	...
	printf(
		"    myPoint roundoff error.  Although these errors are rare, don't use the -X\n"
	);
	...
	printf(
		"  you have a myPoint lying in the middle of a segment?  Triangle sometimes\n");
	printf(
	...
	printf(
		"  one long segment connecting the leftmost myPoint to the rightmost myPoint,\n");
	printf(
	...
	printf("\n");
	printf("    http://www.cs.cmu.edu/~quake/myTriangle.html\n");
	printf("\n");
	printf("A Brief Plea:\n");
	printf("\n");
	printf(
...
void reporterror(const char* lpszerror)
{
	throw lpszerror;
}

void internalerror(const char* lpszerror)
{
	char error[1024];
	sprintf(error, "%s\n%s", lpszerror,
		" ");
...
#define STARTINDEX 1
			...
			for (j = STARTINDEX; argv[i][j] != '\0'; j++)
			{
				...
				if (argv[i][j] == 'a')
				{
						...
						(argv[i][j + 1] == '.'))
					{
						...
						if (maxarea <= 0.0)
						{
							reporterror("Error:  Maximum area must be greater than zero.");
							//reporterror("Error:  Maximum area must be greater than zero.\n");
							//exit(1);
						}
					}
	...
	{
		reporterror("Error:  You cannot use the -I switch when refining a triangulation.");
		//printf(
...
/*****************************************************************************/
/*                                                                           */
/*  printtriangle()   Print out the details of a myTriangle/edge handle.       */
/*                                                                           */
/*  I originally wrote this procedure to simplify debugging; it can be       */
/*  called directly from the debugger, and presents information about a      */
/*  myTriangle/edge handle in digestible form.  It's also used when the        */
	...
	struct triedge* t)
{
	...
	printf("myTriangle x%lx with orientation %d:\n", (ULONG_PTR)t->tri,
		t->orient);
	...
	org(*t, printpoint);
	if (printpoint == (myPoint)NULL)
	{
		printf("    Origin[%d] = NULL\n", (t->orient + 1) % 3 + 3);
	}
	else
		printf("    Origin[%d] = x%lx  (%.12g, %.12g)\n",
		(t->orient + 1) % 3 + 3, (ULONG_PTR)printpoint,
	...
	if (printpoint == (myPoint)NULL)
	{
		...
		(t->orient + 2) % 3 + 3, (ULONG_PTR)printpoint,
			printpoint[0], printpoint[1]);
	apex(*t, printpoint);
	if (printpoint == (myPoint)NULL)
	{
		...
		printf("    Apex  [%d] = x%lx  (%.12g, %.12g)\n",
			t->orient + 3, (ULONG_PTR)printpoint,
			printpoint[0], printpoint[1]);
	if (useshelles)
	{
	...
	struct edge* s)
{
	...
	printf("shell edge x%lx with orientation %d and mark %d:\n",
		(ULONG_PTR)s->sh, s->shorient, mark(*s));
	...
	if (printpoint == (myPoint)NULL)
	{
		...
		printf("    Origin[%d] = x%lx  (%.12g, %.12g)\n",
			2 + s->shorient, (ULONG_PTR)printpoint,
			printpoint[0], printpoint[1]);
	sdest(*s, printpoint);
	if (printpoint == (myPoint)NULL)
	{
		...
		printf("    Dest  [%d] = x%lx  (%.12g, %.12g)\n",
			3 - s->shorient, (ULONG_PTR)printpoint,
			printpoint[0], printpoint[1]);
...
/*  collection of words, and either pointers or floating-myPoint values are    */
	...
	int alignment)
{
	...
	/*   pointer (to myPoint to the next block) are allocated, as well as space */
	...
	{
		reporterror("Error:  Out of memory.");
		//exit(1);
	}
...
/*****************************************************************************/
/*                                                                           */
/*  dummyinit()   Initialize the myTriangle that fills "outer space" and the   */
/*                omnipresent shell edge.                                    */
/*                                                                           */
/*  The myTriangle that fills "outer space", called `dummytri', is pointed to  */
/*  by every myTriangle and shell edge on a boundary (be it outer or inner) of */
/*  the triangulation.  Also, `dummytri' points to one of the triangles on   */
/*  the convex hull (until the holes and concavities are carved), making it  */
/*  possible to find a starting myTriangle for myPoint location.                 */
/*                                                                           */
/*  The omnipresent shell edge, `dummysh', is pointed to by every myTriangle   */
	...
	int shellewords)
{
	...
	/* Set up `dummytri', the `myTriangle' that occupies "outer space". */
	dummytribase = (myTriangle*)malloc(triwords * sizeof(myTriangle)
		+ triangles.alignbytes);
	if (dummytribase == (myTriangle*)NULL)
	{
		reporterror("Error:  Out of memory.");
		//exit(1);
	}
	/* Align `dummytri' on a `triangles.alignbytes'-byte boundary. */
	alignptr = (ULONG_PTR)dummytribase;
	dummytri = (myTriangle*)
		(alignptr + (ULONG_PTR)triangles.alignbytes
			- (alignptr % (ULONG_PTR)triangles.alignbytes));
	/* Initialize the three adjoining triangles to be "outer space".  These  */
	/*   will eventually be changed by various bonding operations, but their */
	/*   values don't really matter, as long as they can legally be          */
	/*   dereferenced.                                                       */
	dummytri[0] = (myTriangle)dummytri;
	dummytri[1] = (myTriangle)dummytri;
	dummytri[2] = (myTriangle)dummytri;
	/* Three NULL vertex points. */
	dummytri[3] = (myTriangle)NULL;
	dummytri[4] = (myTriangle)NULL;
	dummytri[5] = (myTriangle)NULL;

	if (useshelles)
	{
		/* Set up `dummysh', the omnipresent "shell edge" pointed to by any      */
		/*   myTriangle side or shell edge end that isn't attached to a real shell */
		/*   edge.                                                               */
		...
		alignptr = (ULONG_PTR)dummyshbase;
		dummysh = (shelle*)
			(alignptr + (ULONG_PTR)shelles.alignbytes
				- (alignptr % (ULONG_PTR)shelles.alignbytes));
		...
		*(int*)(dummysh + 6) = 0;
		...
		dummytri[6] = (myTriangle)dummysh;
		dummytri[7] = (myTriangle)dummysh;
		dummytri[8] = (myTriangle)dummysh;
	}
}

/*****************************************************************************/
/*                                                                           */
/*  initializepointpool()   Calculate the size of the myPoint data structure   */
/*                          and initialize its memory pool.                  */
/*                                                                           */
/*  This routine also computes the `pointmarkindex' and `point2triindex'     */
/*  indices used to find values within each myPoint.                           */
/*                                                                           */
/*****************************************************************************/
...
void initializepointpool()
{
	...
	/* The index within each myPoint at which the boundary marker is found.  */
	/*   Ensure the myPoint marker is aligned to a sizeof(int)-byte address. */
	pointmarkindex = ((mesh_dim + nextras) * sizeof(REAL) + sizeof(int) - 1)
		/ sizeof(int);
	pointsize = (pointmarkindex + 1) * sizeof(int);
	if (poly)
	{
		/* The index within each myPoint at which a myTriangle pointer is found.   */
		/*   Ensure the pointer is aligned to a sizeof(myTriangle)-byte address. */
		point2triindex = (pointsize + sizeof(myTriangle) - 1) / sizeof(myTriangle);
		pointsize = (point2triindex + 1) * sizeof(myTriangle);
	}
	/* Initialize the pool of points. */
	poolinit(&points, pointsize, POINTPERBLOCK,
		(sizeof(REAL) >= sizeof(myTriangle)) ? FLOATINGPOINT : POINTER, 0);
}

/*****************************************************************************/
/*                                                                           */
/*  initializetrisegpools()   Calculate the sizes of the myTriangle and shell  */
/*                            edge data structures and initialize their      */
/*                            memory pools.                                  */
/*                                                                           */
/*  This routine also computes the `highorderindex', `elemattribindex', and  */
/*  `areaboundindex' indices used to find values within each myTriangle.       */
/*                                                                           */
/*****************************************************************************/
...
void initializetrisegpools()
{
	...
	/* The index within each myTriangle at which the extra nodes (above three)  */
	/*   associated with high order elements are found.  There are three      */
	/*   pointers to other triangles, three pointers to corners, and possibly */
	/*   three pointers to shell edges before the extra nodes.                */
	highorderindex = 6 + (useshelles * 3);
	/* The number of bytes occupied by a myTriangle. */
	trisize = ((order + 1) * (order + 2) / 2 + (highorderindex - 3)) *
		sizeof(myTriangle);
	/* The index within each myTriangle at which its attributes are found, */
	/*   where the index is measured in REALs.                           */
	elemattribindex = (trisize + sizeof(REAL) - 1) / sizeof(REAL);
	/* The index within each myTriangle at which the maximum area constraint  */
	/*   is found, where the index is measured in REALs.  Note that if the  */
	/*   `regionattrib' flag is set, an additional attribute will be added. */
	areaboundindex = elemattribindex + eextras + regionattrib;
	/* If myTriangle attributes or an area bound are needed, increase the number */
	/*   of bytes occupied by a myTriangle.                                      */
	if (vararea)
	{
	...
	/* If a Voronoi diagram or myTriangle neighbor graph is requested, make    */
	/*   sure there's room to store an integer index in each myTriangle.  This */
	...
	if ((voronoi || neighbors) &&
		(trisize < 6 * sizeof(myTriangle) + sizeof(int)))
	{
		trisize = 6 * sizeof(myTriangle) + sizeof(int);
	}
	/* Having determined the memory size of a myTriangle, initialize the pool. */
	poolinit(&triangles, trisize, TRIPERBLOCK, POINTER, 4);

	if (useshelles)
	{
		/* Initialize the pool of shell edges. */
		poolinit(&shelles, 6 * sizeof(myTriangle) + sizeof(int), SHELLEPERBLOCK,
			POINTER, 4);

		/* Initialize the "outer space" myTriangle and omnipresent shell edge. */
		dummyinit(triangles.itemwords, shelles.itemwords);
	}
	else
	{
		/* Initialize the "outer space" myTriangle. */
...
/*****************************************************************************/
/*                                                                           */
/*  triangledealloc()   Deallocate space for a myTriangle, marking it dead.    */
/*                                                                           */
/*****************************************************************************/

void triangledealloc(
	myTriangle* dyingtriangle)
{
	/* Set myTriangle's vertices to NULL.  This makes it possible to        */
	/*   detect dead triangles when traversing the list of all triangles. */
	dyingtriangle[3] = (myTriangle)NULL;
	dyingtriangle[4] = (myTriangle)NULL;
	dyingtriangle[5] = (myTriangle)NULL;
...
myTriangle* triangletraverse()
{
	myTriangle* newtriangle;

	do
	{
		newtriangle = (myTriangle*)traverse(&triangles);
		if (newtriangle == (myTriangle*)NULL)
		{
			return (myTriangle*)NULL;
		}
	} while (newtriangle[3] == (myTriangle)NULL);          /* Skip dead ones. */
	return newtriangle;
}
...
/*****************************************************************************/
/*                                                                           */
/*  pointdealloc()   Deallocate space for a myPoint, marking it dead.          */
/*                                                                           */
/*****************************************************************************/
...
void pointdealloc(
	myPoint dyingpoint)
{
	/* Mark the myPoint as dead.  This makes it possible to detect dead points */
...
myPoint pointtraverse()
{
	myPoint newpoint;

	do
	{
		newpoint = (myPoint)traverse(&points);
		if (newpoint == (myPoint)NULL)
		{
			return (myPoint)NULL;
		}
...
/*****************************************************************************/
/*                                                                           */
/*  getpoint()   Get a specific myPoint, by number, from the list.             */
/*                                                                           */
/*  The first myPoint is number 'firstnumber'.                                 */
/*                                                                           */
	...
	int number)
{
	...
	/* Now find the right myPoint. */
	alignptr = (ULONG_PTR)(getblock + 1);
	foundpoint = (myPoint)(alignptr + (ULONG_PTR)points.alignbytes
		- (alignptr % (ULONG_PTR)points.alignbytes));
...
/*****************************************************************************/
/*                                                                           */
/*  maketriangle()   Create a new myTriangle with orientation zero.            */
/*                                                                           */
/*****************************************************************************/
	...
	struct triedge* newtriedge)
{
	...
	newtriedge->tri = (myTriangle*)poolalloc(&triangles);
	/* Initialize the three adjoining triangles to be "outer space". */
	newtriedge->tri[0] = (myTriangle)dummytri;
	newtriedge->tri[1] = (myTriangle)dummytri;
	newtriedge->tri[2] = (myTriangle)dummytri;
	/* Three NULL vertex points. */
	newtriedge->tri[3] = (myTriangle)NULL;
	newtriedge->tri[4] = (myTriangle)NULL;
	newtriedge->tri[5] = (myTriangle)NULL;
	/* Initialize the three adjoining shell edges to be the omnipresent */
	/*   shell edge.                                                    */
	if (useshelles)
	{
		newtriedge->tri[6] = (myTriangle)dummysh;
		newtriedge->tri[7] = (myTriangle)dummysh;
		newtriedge->tri[8] = (myTriangle)dummysh;
	}
	for (i = 0; i < eextras; i++)
	{
...
/*  floating-myPoint arithmetic.  `epsilon' bounds the relative roundoff       */
/*  error.  It is used for floating-myPoint error analysis.                    */
/*                                                                           */
/*  `splitter' is used to split floating-myPoint numbers into two half-        */
/*  length significands for exact multiplication.                            */
/*                                                                           */
/*  I imagine that a highly optimizing compiler might be too smart for its   */
/*  own good, and somehow cause this routine to fail, if it pretends that    */
/*  floating-myPoint arithmetic is too much like real arithmetic.              */
/*                                                                           */
...
void exactinit()
{
	...
	if (verbose > 1)
	{
		printf("Floating myPoint roundoff is of magnitude %.17g\n", epsilon);
		printf("Floating myPoint splitter is %.17g\n", splitter);
	}
...
/*                       myTriangle defined by the three points.               */
/*                                                                           */
...
/*****************************************************************************/
/*                                                                           */
/*  incircle()   Return a positive value if the myPoint pd lies inside the     */
	...
	REAL permanent)
{
	...
	if (adxtail != 0.0)
	{
		...
		temp16alen = scale_expansion_zeroelim(axtbclen, axtbc, 2.f * adx,
			temp16a);
	...
	if (adytail != 0.0)
	{
		...
		temp16alen = scale_expansion_zeroelim(aytbclen, aytbc, 2.f * ady,
			temp16a);
	...
	if (bdxtail != 0.0)
	{
		...
		temp16alen = scale_expansion_zeroelim(bxtcalen, bxtca, 2.f * bdx,
			temp16a);
	...
	if (bdytail != 0.0)
	{
		...
		temp16alen = scale_expansion_zeroelim(bytcalen, bytca, 2.f * bdy,
			temp16a);
	...
	if (cdxtail != 0.0)
	{
		...
		temp16alen = scale_expansion_zeroelim(cxtablen, cxtab, 2.f * cdx,
			temp16a);
	...
	if (cdytail != 0.0)
	{
		...
		temp16alen = scale_expansion_zeroelim(cytablen, cytab, 2.f * cdy,
			temp16a);
	...
	if ((adxtail != 0.0) || (adytail != 0.0))
	{
		...
		if (adxtail != 0.0)
		{
			...
			temp32alen = scale_expansion_zeroelim(axtbctlen, axtbct, 2.f * adx,
				temp32a);
			...
			temp16alen = scale_expansion_zeroelim(axtbcttlen, axtbctt, 2.f * adx,
				temp16a);
		...
		if (adytail != 0.0)
		{
			...
			temp32alen = scale_expansion_zeroelim(aytbctlen, aytbct, 2.f * ady,
				temp32a);
			temp48len = fast_expansion_sum_zeroelim(temp16alen, temp16a,
				temp32alen, temp32a, temp48);
			finlength = fast_expansion_sum_zeroelim(finlength, finnow, temp48len,
			...
			temp16alen = scale_expansion_zeroelim(aytbcttlen, aytbctt, 2.f * ady,
				temp16a);
	...
	if ((bdxtail != 0.0) || (bdytail != 0.0))
	{
		...
		if (bdxtail != 0.0)
		{
			...
			temp32alen = scale_expansion_zeroelim(bxtcatlen, bxtcat, 2.f * bdx,
				temp32a);
			temp48len = fast_expansion_sum_zeroelim(temp16alen, temp16a,
				temp32alen, temp32a, temp48);
			finlength = fast_expansion_sum_zeroelim(finlength, finnow, temp48len,
			...
			temp16alen = scale_expansion_zeroelim(bxtcattlen, bxtcatt, 2.f * bdx,
				temp16a);
		...
		if (bdytail != 0.0)
		{
			...
			temp32alen = scale_expansion_zeroelim(bytcatlen, bytcat, 2.f * bdy,
				temp32a);
			temp48len = fast_expansion_sum_zeroelim(temp16alen, temp16a,
				temp32alen, temp32a, temp48);
			finlength = fast_expansion_sum_zeroelim(finlength, finnow, temp48len,
			...
			temp16alen = scale_expansion_zeroelim(bytcattlen, bytcatt, 2.f * bdy,
				temp16a);
	...
	if ((cdxtail != 0.0) || (cdytail != 0.0))
	{
		...
		if (cdxtail != 0.0)
		{
			...
			temp32alen = scale_expansion_zeroelim(cxtabtlen, cxtabt, 2.f * cdx,
				temp32a);
			temp48len = fast_expansion_sum_zeroelim(temp16alen, temp16a,
				temp32alen, temp32a, temp48);
			finlength = fast_expansion_sum_zeroelim(finlength, finnow, temp48len,
			...
			temp16alen = scale_expansion_zeroelim(cxtabttlen, cxtabtt, 2.f * cdx,
				temp16a);
		...
		if (cdytail != 0.0)
		{
			...
			temp32alen = scale_expansion_zeroelim(cytabtlen, cytabt, 2.f * cdy,
				temp32a);
			temp48len = fast_expansion_sum_zeroelim(temp16alen, temp16a,
				temp32alen, temp32a, temp48);
			finlength = fast_expansion_sum_zeroelim(finlength, finnow, temp48len,
			...
			temp16alen = scale_expansion_zeroelim(cytabttlen, cytabtt, 2.f * cdy,
				temp16a);
...
void triangleinit()
{
	...
	recenttri.tri = (myTriangle*)NULL;   /* No myTriangle has been visited yet. */
...
void checkmesh()
{
	...
	myPoint triorg, tridest, triapex;
	myPoint oppoorg, oppodest;
	int horrors;
	int saveexact;
	myTriangle ptr;                         /* Temporary variable used by sym(). */
	...
	while (triangleloop.tri != (myTriangle*)NULL)
	{
		/* Check all three edges of the myTriangle. */
			...
			triangleloop.orient++)
		{
			...
			{
				/* Test if the myTriangle is flat or inverted. */
			...
			/* Find the neighboring myTriangle on this edge. */
			sym(triangleloop, oppotri);
			if (oppotri.tri != dummytri)
			{
				/* Check that the myTriangle's neighbor knows it's a neighbor. */
				sym(oppotri, oppooppotri);
				if ((triangleloop.tri != oppooppotri.tri)
					|| (triangleloop.orient != oppooppotri.orient))
				{
					printf("  !! !! Asymmetric myTriangle-myTriangle bond:\n");
					if (triangleloop.tri == oppooppotri.tri)
					{
						printf("   (Right myTriangle, wrong orientation)\n");
					}
					printf("    First ");
					printtriangle(&triangleloop);
...
void checkdelaunay()
{
	...
	while (triangleloop.tri != (myTriangle*)NULL)
	{
			...
			triangleloop.orient++)
		{
			...
			/*   adjoining myTriangle whose pointer is larger (to ensure that */
			/*   each pair isn't tested twice).                             */
			shouldbedelaunay = (oppotri.tri != dummytri)
				&& (triapex != (myPoint)NULL) && (oppoapex != (myPoint)NULL)
...
/*****************************************************************************/
/*                                                                           */
/*  enqueuebadtri()   Add a bad myTriangle to the end of a queue.              */
/*                                                                           */
	...
	myPoint insdest)
{
	...
	if (verbose > 2)
	{
		printf("  Queueing bad myTriangle:\n");
		printf("    (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n", insorg[0],
			insorg[1], insdest[0], insdest[1], insapex[0], insapex[1]);
	}
	/* Allocate space for the bad myTriangle. */
	...
	/* Determine the appropriate queue to put the bad myTriangle into. */
	if (angle > 0.6)
	{
	...
	{
		/* It's not a bad angle; put the myTriangle in the lowest-priority queue. */
		queuenumber = 0;
	}
	/* Add the myTriangle to the end of a queue. */
...
/*****************************************************************************/
/*                                                                           */
/*  dequeuebadtri()   Remove a myTriangle from the front of the queue.         */
/*                                                                           */
/*****************************************************************************/
...
struct badface* dequeuebadtri()
{
	...
	for (queuenumber = 63; queuenumber >= 0; queuenumber--)
	{
		...
		{
			/* Remove the myTriangle from the queue. */
...
/*                                                                           */
/*  An encroached segment is an unflippable edge that has a myPoint in its     */
	...
	struct edge* testedge)
{
	...
	int addtolist;
	int sides;
	myPoint eorg, edest, eapex;
	myTriangle ptr;                     /* Temporary variable used by stpivot(). */
...
/*                                                                           */
/*  Tests a myTriangle to see if it satisfies the minimum angle condition and  */
/*  the maximum area condition.  Triangles that aren't up to spec are added  */
/*  to the bad myTriangle queue.                                               */
/*                                                                           */
/*****************************************************************************/
	...
	struct triedge* testtri)
{
	...
	dxao2 = dxao * dxao;
	dyao2 = dyao * dyao;
	/* Find the lengths of the myTriangle's three edges. */
	apexlen = dxod2 + dyod2;
	orglen = dxda2 + dyda2;
	destlen = dxao2 + dyao2;
	...
	if ((edge1.sh != dummysh) && (edge2.sh != dummysh))
	{
		...
		if ((angle > 0.9924) && !quiet)                /* Roughly 5 degrees. */
		{
			...
			printf(
				"Warning:  Small angle (%.4g degrees) between segments at myPoint\n",
				angle);
			printf("  (%.12g, %.12g)\n", anglevertex[0], anglevertex[1]);
		}
		/* Don't add this bad myTriangle to the list; there's nothing that */
	...
	if (angle > goodangle)
	{
		/* Add this myTriangle to the list of bad triangles. */
...
/*                  the speed of myPoint location for segment insertion.       */
/*                                                                           */
/*  Traverses all the triangles, and provides each corner of each myTriangle   */
/*  with a pointer to that myTriangle.  Of course, pointers will be            */
/*  overwritten by other pointers because (almost) each myPoint is a corner    */
/*  of several triangles, but in the end every myPoint will myPoint to some      */
/*  myTriangle that contains it.                                               */
/*                                                                           */
/*****************************************************************************/
...
void makepointmap()
{
	...
	while (triangleloop.tri != (myTriangle*)NULL)
	{
		/* Check all three points of the myTriangle. */
		for (triangleloop.orient = 0; triangleloop.orient < 3;
			triangleloop.orient++)
		{
			org(triangleloop, triorg);
			setpoint2tri(triorg, encode(triangleloop));
			g_pTrMap.insert(std::make_pair(triorg, triorg));
		}
		triangleloop.tri = triangletraverse();
	}
}
...
/*****************************************************************************/
/*                                                                           */
/*  preciselocate()   Find a myTriangle or edge containing a given myPoint.      */
/*                                                                           */
...
/*                                                                           */
/*  (1)  To try to find the location to insert a new myPoint.  Normally, we    */
/*       know an edge that the myPoint is strictly to the left of.  In the     */
/*       incremental Delaunay algorithm, that edge is a bounding box edge.   */
/*       In Ruppert's Delaunay refinement algorithm for quality meshing,     */
/*       that edge is the shortest edge of the myTriangle whose circumcenter   */
/*       is being inserted.                                                  */
/*                                                                           */
/*  (2)  To try to find an existing myPoint.  In this case, any edge on the    */
...
/*                                                                           */
/*  On completion, `searchtri' is a myTriangle that contains `searchpoint'.    */
/*                                                                           */
/*  This implementation differs from that given by Guibas and Stolfi.  It    */
/*  walks from myTriangle to myTriangle, crossing an edge only if `searchpoint'  */
/*  is on the other side of the line containing that edge.  After entering   */
/*  a myTriangle, there are two edges by which one can leave that myTriangle.    */
...
/*  myPoint location routine (which I originally used), which tends to spiral  */
/*  in toward its target.                                                    */
/*                                                                           */
/*  Returns ONVERTEX if the myPoint lies on an existing vertex.  `searchtri'   */
/*  is a handle whose origin is the existing vertex.                         */
/*                                                                           */
/*  Returns ONEDGE if the myPoint lies on a mesh edge.  `searchtri' is a       */
/*  handle whose primary edge is the edge on which the myPoint lies.           */
/*                                                                           */
/*  Returns INTRIANGLE if the myPoint lies strictly within a myTriangle.         */
/*  `searchtri' is a handle on the myTriangle that contains the myPoint.         */
/*                                                                           */
/*  Returns OUTSIDE if the myPoint lies outside the mesh.  `searchtri' is a    */
/*  handle whose primary edge the myPoint is to the right of.  This might      */
/*  occur when the circumcenter of a myTriangle falls just slightly outside    */
/*  the mesh due to floating-myPoint roundoff error.  It also occurs when      */
/*  seeking a hole or region myPoint that a foolish user has placed outside    */
/*  the mesh.                                                                */
/*                                                                           */
/*  WARNING:  This routine is designed for convex triangulations, and will   */
/*  not generally work after the holes and concavities have been carved.     */
/*  However, it can still be used to find the circumcenter of a myTriangle, as */
/*  long as the search is begun from the myTriangle in question.               */
/*                                                                           */
/*****************************************************************************/
	...
	struct triedge* searchtri)
{
	...
	if (verbose > 2)
	{
		printf("  Searching for myPoint (%.12g, %.12g).\n",
	...
	while (1)
	{
		...
		/* Check whether the apex is the myPoint we seek. */
		if ((fapex[0] == searchpoint[0]) && (fapex[1] == searchpoint[1]))
		{
			lprevself(*searchtri);
			return ONVERTEX;
		}
		/* Does the myPoint lie on the other side of the line defined by the */
		/*   myTriangle edge opposite the myTriangle's destination?            */
		destorient = counterclockwise(forg, fapex, searchpoint);
		/* Does the myPoint lie on the other side of the line defined by the */
		/*   myTriangle edge opposite the myTriangle's origin?                 */
		...
		else
		{
			...
			{
				/* The myPoint we seek must be on the boundary of or inside this */
				/*   myTriangle.                                                 */
				if (destorient == 0.0)
				{
					lprevself(*searchtri);
		...
		/* Move to another myTriangle.  Leave a trace `backtracktri' in case */
		/*   floating-myPoint roundoff or some such bogey causes us to walk  */
		...
		if (searchtri->tri == dummytri)
		{
			...
			fdest = swappoint;
			apex(*searchtri, fapex);
			/* Check if the myPoint really is beyond the triangulation boundary. */
			destorient = counterclockwise(forg, fapex, searchpoint);
			orgorient = counterclockwise(fapex, fdest, searchpoint);
...
/*****************************************************************************/
/*                                                                           */
/*  locate()   Find a myTriangle or edge containing a given myPoint.             */
/*                                                                           */
/*  Searching begins from one of:  the input `searchtri', a recently         */
/*  encountered myTriangle `recenttri', or from a myTriangle chosen from a       */
/*  random sample.  The choice is made by determining which myTriangle's       */
/*  origin is closest to the myPoint we are searcing for.  Normally,           */
	...
	struct triedge* searchtri)
{
	...
	if (verbose > 2)
	{
		printf("  Randomly sampling for a myTriangle near myPoint (%.12g, %.12g).\n",
			searchpoint[0], searchpoint[1]);
	}
	/* Record the distance from the suggested starting myTriangle to the */
	/*   myPoint we seek.                                                */
	org(*searchtri, torg);
	...
	if (verbose > 2)
	{
		printf("    Boundary myTriangle has origin (%.12g, %.12g).\n",
			torg[0], torg[1]);
	}

	/* If a recently encountered myTriangle has been recorded and has not been */
	/*   deallocated, test it as a good starting myPoint.                      */
	if (recenttri.tri != (myTriangle*)NULL)
	{
		if (recenttri.tri[3] != (myTriangle)NULL)
		{
			org(recenttri, torg);
			...
			if (dist < searchdist)
			{
				...
				if (verbose > 2)
				{
					printf("    Choosing recent myTriangle with origin (%.12g, %.12g).\n",
						torg[0], torg[1]);
				}
			}
		}
	...
	for (i = 0; i < sampleblocks; i++)
	{
		...
		firsttri = (myTriangle*)(alignptr + (ULONG_PTR)triangles.alignbytes
			- (alignptr % (ULONG_PTR)triangles.alignbytes));
		for (j = 0; j < samplesperblock; j++)
		{
			...
			sampletri.tri = (myTriangle*)
				(firsttri + (samplenum * triangles.itemwords));
			if (sampletri.tri[3] != (myTriangle)NULL)
			{
				org(sampletri, torg);
				dist = (searchpoint[0] - torg[0]) * (searchpoint[0] - torg[0])
					+ (searchpoint[1] - torg[1]) * (searchpoint[1] - torg[1]);
				if (dist < searchdist)
				{
					...
					if (verbose > 2)
					{
						printf("    Choosing myTriangle with origin (%.12g, %.12g).\n",
							torg[0], torg[1]);
					}
				}
			}
	...
	/* Check the starting myTriangle's vertices. */
	if ((torg[0] == searchpoint[0]) && (torg[1] == searchpoint[1]))
	{
		return ONVERTEX;
	}
	...
	int shellemark)                           /* Marker for the new shell edge. */
{
	...
	if (newshelle.sh == dummysh)
	{
		...
		/*   Note that the facing myTriangle `oppotri' might be equal to        */
...
/*  shared edge ab lies in a horizontal plane, with the myPoint b on the left  */
/*  and the myPoint a on the right.  The myPoint c lies below the edge, and the  */
/*  myPoint d lies above the edge.  The `flipedge' handle holds the edge ab    */
/*  of myTriangle abc, and is directed left, from vertex a to vertex b.        */
/*                                                                           */
...
/*  dc of myTriangle dca, and is directed down, from vertex d to vertex c.     */
...
void flip(
	struct triedge* flipedge)                   /* Handle for the myTriangle abc. */
{
	...
	myPoint leftpoint, rightpoint, botpoint;
	myPoint farpoint;
	myTriangle ptr;                         /* Temporary variable used by sym(). */
	shelle sptr;                      /* Temporary variable used by tspivot(). */
	...
	/* New myPoint assignments for the rotated quadrilateral. */
...
/*                 property.                                                 */
/*                                                                           */
/*  The myPoint `insertpoint' is located.  If `searchtri.tri' is not NULL,     */
/*  the search for the containing myTriangle begins from `searchtri'.  If      */
/*  `searchtri.tri' is NULL, a full myPoint location procedure is called.      */
/*  If `insertpoint' is found inside a myTriangle, the myTriangle is split into  */
...
/*                                                                           */
/*  If the calling routine wishes to split a segment by inserting a myPoint in */
/*  it, the parameter `splitedge' should be that segment.  In this case,     */
/*  `searchtri' MUST be the myTriangle handle reached by pivoting from that    */
/*  segment; no myPoint location is done.                                      */
/*                                                                           */
/*  `segmentflaws' and `triflaws' are flags that indicate whether or not     */
/*  there should be checks for the creation of encroached segments or bad    */
/*  quality faces.  If a newly inserted myPoint encroaches upon segments,      */
...
/*                                                                           */
/*  If a duplicate myPoint or violated segment does not prevent the myPoint      */
/*  from being inserted, the return value will be ENCROACHINGPOINT if the    */
/*  myPoint encroaches upon a segment (and checking is enabled), or            */
	...
	int triflaws)
{
	...
	myPoint first;
	myPoint leftpoint, rightpoint, botpoint, toppoint, farpoint;
	REAL attrib;
	REAL area;
	...
	{
		/* Find the location of the myPoint to be inserted.  Check if a good */
		/*   starting myTriangle has already been provided by the caller.    */
		if (searchtri->tri == (myTriangle*)NULL)
		{
			/* Find a boundary myTriangle. */
			horiz.tri = dummytri;
			horiz.orient = 0;
			symself(horiz);
			/* Search for a myTriangle containing `insertpoint'. */
			intersect = locate(insertpoint, &horiz);
		}
		else
		{
			/* Start searching from the myTriangle provided by the caller. */
	...
	{
		/* The calling routine provides the edge in which the myPoint is inserted. */
		triedgecopy(*searchtri, horiz);
		intersect = ONEDGE;
	}
	if (intersect == ONVERTEX)
	{
		/* There's already a vertex there.  Return in `searchtri' a myTriangle */
	...
	if ((intersect == ONEDGE) || (intersect == OUTSIDE))
	{
		...
		if (checksegments && (splitedge == (struct edge*) NULL))
		{
			...
			if (brokenshelle.sh != dummysh)
			{
				...
				/* Return a handle whose primary edge contains the myPoint, */
		...
		/* Insert the myPoint on an edge, dividing one myTriangle into two (if */
		/*   the edge lies on a boundary) or two triangles into four.      */
		lprev(horiz, botright);
		sym(botright, botrcasing);
		sym(horiz, topright);
		/* Is there a second myTriangle?  (Or does this edge lie on a boundary?) */
		...
		for (i = 0; i < eextras; i++)
		{
			/* Set the element attributes of a new myTriangle. */
			setelemattribute(newbotright, i, elemattribute(botright, i));
		}
		if (vararea)
		{
			/* Set the area constraint of a new myTriangle. */
		...
		if (mirrorflag)
		{
			...
			for (i = 0; i < eextras; i++)
			{
				/* Set the element attributes of another new myTriangle. */
				setelemattribute(newtopright, i, elemattribute(topright, i));
			}
			if (vararea)
			{
				/* Set the area constraint of another new myTriangle. */
				setareabound(newtopright, areabound(topright));
			}
		}

		/* There may be shell edges that need to be bonded */
		/*   to the new myTriangle(s).                       */
		if (checksegments)
		{
			tspivot(botright, botrshelle);
			if (botrshelle.sh != dummysh)
		...
		/* Bond the new myTriangle(s) to the surrounding triangles. */
		...
		if (counterclockwise(rightpoint, leftpoint, botpoint) < 0.0)
		{
			...
			printf("  Clockwise myTriangle prior to edge myPoint insertion (bottom).\n");
		}
		if (mirrorflag)
		{
			...
			{
				printf("Internal error in insertsite():\n");
				printf("  Clockwise myTriangle prior to edge myPoint insertion (top).\n");
			}
			if (counterclockwise(rightpoint, toppoint, insertpoint) < 0.0)
			{
				printf("Internal error in insertsite():\n");
				printf("  Clockwise myTriangle after edge myPoint insertion (top right).\n");
			}
			if (counterclockwise(toppoint, leftpoint, insertpoint) < 0.0)
			{
				printf("Internal error in insertsite():\n");
				printf("  Clockwise myTriangle after edge myPoint insertion (top left).\n");
			}
		}
		if (counterclockwise(leftpoint, botpoint, insertpoint) < 0.0)
		{
			printf("Internal error in insertsite():\n");
			printf("  Clockwise myTriangle after edge myPoint insertion (bottom left).\n");
		}
		if (counterclockwise(botpoint, rightpoint, insertpoint) < 0.0)
		{
			printf("Internal error in insertsite():\n");
			printf("  Clockwise myTriangle after edge myPoint insertion (bottom right).\n");
		}
#endif /* _DEBUG */
		if (verbose > 2)
		{
	...
	{
		/* Insert the myPoint in a myTriangle, splitting it into three. */
		lnext(horiz, botleft);
		lprev(horiz, botright);
		sym(botleft, botlcasing);
		sym(botright, botrcasing);
		...
		{
			printf("Internal error in insertsite():\n");
			printf("  Clockwise myTriangle prior to myPoint insertion.\n");
		}
		if (counterclockwise(rightpoint, leftpoint, insertpoint) < 0.0)
		{
			printf("Internal error in insertsite():\n");
			printf("  Clockwise myTriangle after myPoint insertion (top).\n");
		}
		if (counterclockwise(leftpoint, botpoint, insertpoint) < 0.0)
		{
			printf("Internal error in insertsite():\n");
			printf("  Clockwise myTriangle after myPoint insertion (left).\n");
		}
		if (counterclockwise(botpoint, rightpoint, insertpoint) < 0.0)
		{
			printf("Internal error in insertsite():\n");
			printf("  Clockwise myTriangle after myPoint insertion (right).\n");
		}
#endif /* _DEBUG */
		if (verbose > 2)
		{
	...
	while (1)
	{
		...
		if (checksegments)
		{
			...
			if (checkshelle.sh != dummysh)
			{
				...
				if (segmentflaws)
				{
					/* Does the new myPoint encroach upon this segment? */
		...
		if (doflip)
		{
			...
			{
				/* Find the myPoint on the other side of the edge. */
				apex(top, farpoint);
					...
					|| (farpoint == infpoint3))
				{
					...
					/*   the circumcircle of the myTriangle `horiz'.           */
					doflip = 0;
				}
				else
				{
...
#ifdef _DEBUG
					if (insertpoint != (myPoint)NULL)
					{
						if (counterclockwise(leftpoint, insertpoint, rightpoint) < 0.0)
						{
							printf("Internal error in insertsite():\n");
							printf("  Clockwise myTriangle prior to edge flip (bottom).\n");
						}
						...
						/*
									if (counterclockwise(rightpoint, farpoint, leftpoint) < 0.0) {
									  printf("Internal error in insertsite():\n");
									  printf("  Clockwise myTriangle prior to edge flip (top).\n");
									}
						*/
						if (counterclockwise(farpoint, leftpoint, insertpoint) < 0.0)
						{
							printf("Internal error in insertsite():\n");
							printf("  Clockwise myTriangle after edge flip (left).\n");
						}
						if (counterclockwise(insertpoint, rightpoint, farpoint) < 0.0)
						{
							printf("Internal error in insertsite():\n");
							printf("  Clockwise myTriangle after edge flip (right).\n");
						}
					}
#endif /* _DEBUG */
					if (verbose > 2)
		...
		if (!doflip)
		{
			...
			if (triflaws)
			{
				/* Check the myTriangle `horiz' for quality. */
				testtriangle(&horiz);
			}
#endif /* not CDT_ONLY */
			/* Look for the next edge around the newly inserted myPoint. */
			lnextself(horiz);
			sym(horiz, testtri);
			/* Check for finishing a complete revolution about the new myPoint, or */
			/*   falling off the edge of the triangulation.  The latter will     */
			/*   happen when a myPoint is inserted at a boundary.                  */
			if ((leftpoint == first) || (testtri.tri == dummytri))
			{
				/* We're done.  Return a myTriangle whose origin is the new myPoint. */
				lnext(horiz, *searchtri);
				lnext(horiz, recenttri);
				return success;
			}
			/* Finish finding the next edge around the newly inserted myPoint. */
			lnext(testtri, horiz);
...
/*                         polygons that result from deletion of a myPoint or  */
...
/*  primary edge of each myTriangle is the edge directed from the origin to    */
/*  the destination; note that this is not the same edge that is a side of   */
/*  the polygon.  `firstedge' is the primary edge of the first myTriangle.     */
/*  From there, the triangles follow in counterclockwise order about the     */
/*  polygon, until `lastedge', the primary edge of the last myTriangle.        */
...
/*  edge flip effectively removes one myTriangle from the fan, committing it   */
/*  to the polygon.  The resulting polygon has one fewer edge.  If `doflip'  */
/*  is set, the final flip will be performed, resulting in a fan of one      */
/*  (useless?) myTriangle.  If `doflip' is not set, the final flip is not      */
/*  performed, resulting in a fan of two triangles, and an unfinished        */
/*  triangular polygon that is not yet filled out with a single myTriangle.    */
/*  On completion of the routine, `lastedge' is the last remaining myTriangle, */
...
/*  rarely a big deal for myPoint deletion, where n is rarely larger than ten, */
	...
	int triflaws)
{
	...
	myPoint leftbasepoint, rightbasepoint;
	myPoint testpoint;
	myPoint bestpoint;
	int bestnumber;
	int i;
	myTriangle ptr;   /* Temporary variable used by sym(), onext(), and oprev(). */
	...
	if (doflip)
	{
		...
		if (triflaws)
		{
			/* Check the quality of the newly committed myTriangle. */
			sym(besttri, testtri);
			testtriangle(&testtri);
		}
#endif /* not CDT_ONLY */
	}
	/* Return the base myTriangle. */
...
/*  to this myPoint is a polygon, for which the Delaunay triangulation is      */
	...
	struct triedge* deltri)
{
	...
	/* Count the degree of the myPoint being deleted. */
	...
	while (!triedgeequal(*deltri, countingtri))
	{
		...
		{
			internalerror("Internal error in deletesite():\nAttempt to delete boundary myPoint.");
		}
#endif /* _DEBUG */
		edgecount++;
	...
	{
		char error[256];
		sprintf(error, "Internal error in deletesite():\n  Point has degree %d.",
			edgecount);
		internalerror(error);
	}
#endif /* _DEBUG */
	if (edgecount > 3)
	{
		...
		/*   adjacent to the myPoint being deleted.  Check the quality of  */
...
/*  and Stolfi.  Switching to a myTriangle-based data structure doubled the    */
/*  speed.  However, I had to think of a few extra tricks to maintain the    */
/*  elegance of the original algorithms.                                     */
/*                                                                           */
/*  The "bounding box" used by my variant of the divide-and-conquer          */
/*  algorithm uses one myTriangle for each edge of the convex hull of the      */
/*  triangulation.  These bounding triangles all share a common apical       */
/*  vertex, which is represented by NULL and which represents nothing.       */
/*  The bounding triangles are linked in a circular fan about this NULL      */
/*  vertex, and the edges on the convex hull of the triangulation appear     */
/*  opposite the NULL vertex.  You might find it easiest to imagine that     */
/*  the NULL vertex is a myPoint in 3D space behind the center of the          */
	...
	int arraysize)
{
	...
	while (left < right)
	{
		/* Search for a myPoint whose x-coordinate is too large for the left. */
		do
		{
			left++;
		...
		/* Search for a myPoint whose x-coordinate is too small for the right. */
		do
		{
			right--;
...
/*  a myTriangle-based, rather than edge-based, data structure.                */
/*                                                                           */
...
/*  step, whether the next "tooth" is a bounding myTriangle of the left hull   */
/*  or the right.  When a bounding myTriangle becomes real, its apex is        */
/*  changed from NULL to a real myPoint.                                       */
/*                                                                           */
	...
	int axis)
{
	...
	myPoint innerleftdest;
	myPoint innerrightorg;
	myPoint innerleftapex, innerrightapex;
	myPoint farleftpt, farrightpt;
	myPoint farleftapex, farrightapex;
	myPoint lowerleft, lowerright;
	myPoint upperleft, upperright;
	myPoint nextapex;
	myPoint checkvertex;
	int changemade;
	int badedge;
	...
	if (dwyer && (axis == 1))
	{
		...
		/* The pointers to the extremal points are shifted to myPoint to the */
		/*   topmost and bottommost myPoint of each hull, rather than the    */
	...
	{
		changemade = 0;
		/* Make innerleftdest the "bottommost" myPoint of the left hull. */
		...
		/* Make innerrightorg the "bottommost" myPoint of the right hull. */
	...
	/* Create the bottom new bounding myTriangle. */
	...
	while (1)
	{
		...
		/*   moving up on the right triangulation might reveal a new myPoint of    */
		...
		{
			/* Create the top new bounding myTriangle. */
		...
		if (!leftfinished)
		{
			...
			if (nextapex != (myPoint)NULL)
			{
				...
				while (badedge)
				{
					...
					/*   left triangulation will have one more boundary myTriangle. */
					lnextself(nextedge);
		...
		if (!rightfinished)
		{
			...
			if (nextapex != (myPoint)NULL)
			{
				...
				{
					/* Eliminate the edge with an edge flip.  As a result, the     */
					/*   right triangulation will have one more boundary myTriangle. */
					lprevself(nextedge);
					sym(nextedge, topcasing);
					lprevself(nextedge);
					sym(nextedge, sidecasing);
...
//VMProgress *			g_triprogress = NULL;
//static int				g_nProgress;
static float			g_ratio;
static int				g_nProcessed;

extern "C" __declspec(dllexport)
/*
	void SetProgress(VOID *progress)
	{
	g_triprogress = (VMProgress *)progress;
	}
*/
void divconqrecurse(
	myPoint* sortarray,
	...
	struct triedge* farright)
{
	...
	if (vertices == 2)
	{
		...
		//		if((long)(g_nProcessed/g_ratio)>g_nProgress){
		//			if(g_triprogress)g_triprogress->StepIt();
		//			g_nProgress++;
		//		}
		//		g_nProcessed +=2;
	...
	{
		/* The triangulation of three vertices is either a myTriangle (with */
	...
	struct triedge* startghost)
{
	...
	/* Find an edge on the convex hull to start myPoint location from. */
	...
	do
	{
		...
		/* Remove a bounding myTriangle from a convex hull myTriangle. */
		dissolve(dissolveedge);
		/* Find the next bounding myTriangle. */
		sym(deadtri, dissolveedge);
		/* Delete the bounding myTriangle. */
...
long divconqdelaunay()
{
	...
	sortarray = (myPoint*)malloc(inpoints * sizeof(myPoint));
	if (sortarray == (myPoint*)NULL)
	{
		reporterror("Error:  Out of memory.");
		//exit(1);
	}
	...
	for (j = 1; j < inpoints; j++)
	{
			...
			&& (sortarray[i][1] == sortarray[j][1]))
		{
			...
			{
				printf(
					"Warning:  A duplicate myPoint at (%.12g, %.12g) appeared and was ignored.\n",
					sortarray[j][0], sortarray[j][1]);
			}
			/*  Commented out - would eliminate myPoint from output .node file, but causes
				a failure if some segment has this myPoint as an endpoint.
	...
	long lNum = (long)sqrt((float)i);
	if (lNum > 100) { lNum = 100; }
	g_ratio = (float)i / lNum;
	//  if(g_triprogress)g_triprogress->StartProgress(lNum);
	//  g_nProgress = 0;
	g_nProcessed = 0;
	divconqrecurse(sortarray, i, 0, &hullleft, &hullright);
	//  if(g_triprogress)g_triprogress->EndProgress();
	free(sortarray);
...
/*****************************************************************************/
/*                                                                           */
/*  boundingbox()   Form an "infinite" bounding myTriangle to insert points    */
/*                  into.                                                    */
/*                                                                           */
/*  The points at "infinity" are assigned finite coordinates, which are used */
/*  by the myPoint location routines, but (mostly) ignored by the Delaunay     */
/*  edge flip routines.                                                      */
/*                                                                           */
/*****************************************************************************/
...
void boundingbox()
{
	...
	infpoint1 = (myPoint)malloc(points.itembytes);
	infpoint2 = (myPoint)malloc(points.itembytes);
	infpoint3 = (myPoint)malloc(points.itembytes);
	if ((infpoint1 == (myPoint)NULL) || (infpoint2 == (myPoint)NULL)
		|| (infpoint3 == (myPoint)NULL))
	{
		reporterror("Error:  Out of memory.");
		//exit(1);
	}
	...
	/*   edge to begin searching (myPoint location) from.           */
	dummytri[0] = (myTriangle)inftri.tri;
	if (verbose > 2)
	{
		printf("  Creating ");
...
/*****************************************************************************/
/*                                                                           */
/*  removebox()   Remove the "infinite" bounding myTriangle, setting boundary  */
/*                markers as appropriate.                                    */
/*                                                                           */
/*  The triangular bounding box has three boundary triangles (one for each   */
/*  side of the bounding box), and a bunch of triangles fanning out from     */
/*  the three bounding box vertices (one myTriangle for each edge of the       */
...
long removebox()
{
	...
	lnextself(nextedge);
	symself(nextedge);
	/* Find a myTriangle (on the boundary of the myPoint set) that isn't */
	/*   a bounding box myTriangle.                                    */
	lprev(nextedge, searchedge);
	symself(searchedge);
	/* Check whether nextedge is another boundary myTriangle */
	/*   adjacent to the first one.                        */
	lnext(nextedge, checkedge);
	symself(checkedge);
	if (checkedge.tri == dummytri)
	{
		/* Go on to the next myTriangle.  There are only three boundary   */
		/*   triangles, and this next myTriangle cannot be the third one, */
		/*   so it's safe to stop here.                                 */
		lprevself(searchedge);
		symself(searchedge);
	}
	/* Find a new boundary edge to search from, as the current search */
	/*   edge lies on a bounding box myTriangle and will be deleted.    */
	dummytri[0] = encode(searchedge);
	hullsize = -2l;
	while (!triedgeequal(nextedge, finaledge))
	{
		...
		/* Disconnect the bounding box myTriangle from the mesh myTriangle. */
		dissolve(dissolveedge);
		lnext(nextedge, deadtri);
		sym(deadtri, nextedge);
		/* Get rid of the bounding box myTriangle. */
		triangledealloc(deadtri.tri);
...
long incrementaldelaunay()
{
	...
	i = 1;
	while (pointloop != (myPoint)NULL)
	{
		/* Find a boundary myTriangle to search from. */
		starttri.tri = (myTriangle*)NULL;
			...
			DUPLICATEPOINT)
		{
			...
			/*  Commented out - would eliminate myPoint from output .node file.
	...
	REAL topy)
{
	...
	searchpoint[1] = topy;
	return splayinsert(splay(splayroot, (myPoint)searchpoint, &dummytri), newkey,
		(myPoint)searchpoint);
}
...
{
	int farrightflag;
	myTriangle ptr;                       /* Temporary variable used by onext(). */
...
long sweeplinedelaunay()
{
	...
	myPoint nextpoint, lastpoint;
	myPoint connectpoint;
	myPoint leftpoint, midpoint, rightpoint;
	...
	lprevself(righttri);
	bond(lefttri, righttri);
	firstpoint = (myPoint)eventheap[0]->eventptr;
	...
	do
	{
		if (heapsize == 0)
		{
			reporterror("Error:  Input points are all identical.");
			//exit(1);
		}
		secondpoint = (myPoint)eventheap[0]->eventptr;
		eventheap[0]->eventptr = (VOID*)freeevents;
		freeevents = eventheap[0];
		eventheapdelete(eventheap, heapsize, 0);
		heapsize--;
	...
	while (heapsize > 0)
	{
		...
		{
			nextpoint = (myPoint)nextevent->eventptr;
...
/*  by forming one stack of triangles for each vertex.  Each myTriangle is on  */
/*  three different stacks simultaneously.  Each myTriangle's shell edge       */
/*  pointers are used to link the items in each stack.  This memory-saving   */
/*  feature makes the code harder to read.  The most important thing to keep */
/*  in mind is that each myTriangle is removed from a stack precisely when     */
/*  the corresponding pointer is adjusted to refer to a shell edge rather    */
/*  than the next myTriangle of the stack.                                     */
/*                                                                           */
/*****************************************************************************/
...

{
	...
	myTriangle* vertexarray;
	myTriangle* prevlink;
	myTriangle nexttri;
	myPoint tdest, tapex;
	myPoint checkdest, checkapex;
	myPoint shorg;
	myPoint killpoint;
	REAL area;
	int corner[3];
	...
	if (incorners < 3)
	{
		reporterror("Error:  Triangles must have at least 3 points.");
		//exit(1);
	}
	eextras = attribs;
#else /* not TRILIBRARY */
	...
	if (elefile == (FILE*)NULL)
	{
		char error[256];
		sprintf(error,
			"  Error:  Cannot access file %s.", elefilename);
		reporterror(error);
		//exit(1);
	}
	/* Read number of triangles, number of points per myTriangle, and */
	/*   number of myTriangle attributes from .ele file.              */
	...
	else
	{
		...
		if (incorners < 3)
		{
			char error[256];
			sprintf(error,
				"Error:  Triangles in %s must have at least 3 points.", elefilename);
			reporterror(error);
			//exit(1);
		}
	}
	...
	for (elementnumber = 1; elementnumber <= inelements; elementnumber++)
	{
		...
		/* Mark the myTriangle as living. */
		triangleloop.tri[3] = (myTriangle)triangleloop.tri;
	}
	...
	if (vararea)
	{
		...
		if (areafile == (FILE*)NULL)
		{
			char error[256];
			sprintf(error,
				"  Error:  Cannot access file %s.", areafilename);
			reporterror(error);
			//exit(1);
		}
		...
		if (areaelements != inelements)
		{
			char error[256];
			sprintf(error,
				"Error:  %s and %s disagree on number of triangles.", elefilename, areafilename);
			reporterror(error);
			//exit(1);
		}
	}
	...
	/* Allocate a temporary array that maps each myPoint to some adjacent  */
	/*   myTriangle.  I took care to allocate all the permanent memory for */
	/*   triangles and shell edges first.                                */
	vertexarray = (myTriangle*)malloc(points.items * sizeof(myTriangle));
	if (vertexarray == (myTriangle*)NULL)
	{
		reporterror("Error:  Out of memory.");
		//exit(1);
	}
	/* Each myPoint is initially unrepresented. */
	for (i = 0; i < points.items; i++)
	{
		vertexarray[i] = (myTriangle)dummytri;
	}
	...
	{
#ifdef TRILIBRARY
		/* Copy the myTriangle's three corners. */
		for (j = 0; j < 3; j++)
		{
			...
			if ((corner[j] < firstnumber) || (corner[j] >= firstnumber + inpoints))
			{
				char error[256];
				sprintf(error,
					"Error:  Triangle %d has an invalid vertex index.", elementnumber);
				reporterror(error);
				//exit(1);
			}
		}
#else /* not TRILIBRARY */
		/* Read myTriangle number and the myTriangle's three corners. */
		stringptr = readline(inputline, elefile, elefilename);
		for (j = 0; j < 3; j++)
		{
			stringptr = findfield(stringptr);
			if (*stringptr == '\0')
			{
				char error[256];
				sprintf(error,
					"Error:  Triangle %d is missing myPoint %d in %s.",
		...
		for (j = 3; j < incorners; j++)
		{
			...
			if (*stringptr != '\0')
			{
				...
				{
					/* Delete the non-corner myPoint if it's not already deleted. */
		...
		/* Read the myTriangle's attributes. */
		for (j = 0; j < eextras; j++)
		{
#ifdef TRILIBRARY
		...
		if (vararea)
		{
			...
			if (*stringptr == '\0')
			{
				area = -1.0;                      /* No constraint on this myTriangle. */
			}
			else
			{
			...
			triangleloop.orient++)
		{
			...
			/* Link the current myTriangle to the next one in the stack. */
			triangleloop.tri[6 + triangleloop.orient] = nexttri;
			/* Push the current myTriangle onto the stack. */
			...
			if (checktri.tri != dummytri)
			{
				...
				do
				{
					...
					/* Find the next myTriangle in the stack. */
	...
	if (poly)
	{
		...
		while (shelleloop.sh != (shelle*)NULL)
		{
			...
			if (*stringptr == '\0')
			{
				char error[256];
				sprintf(error,
					"Error:  Segment %d has no endpoints in %s.", segmentnumber, polyfilename);
				reporterror(error);
				//exit(1);
			}
			else
			...
			if (*stringptr == '\0')
			{
				char error[256];
				sprintf(error,
					"Error:  Segment %d is missing its second endpoint in %s.", segmentnumber, polyfilename);
				reporterror(error);
				//exit(1);
			}
			else
			...
			for (j = 0; j < 2; j++)
			{
				if ((end[j] < firstnumber) || (end[j] >= firstnumber + inpoints))
				{
					char error[256];
					sprintf(error,
						"Error:  Segment %d has an invalid vertex index.", segmentnumber);
					reporterror(error);
					//exit(1);
				}
			}
				...
				shelleloop.shorient++)
			{
				...
				/*   comparing each myTriangle's destination with the shell edge;   */
				/*   each myTriangle's apex is handled through a different vertex.  */
				/*   Because each myTriangle appears on three vertices' lists, each */
				/*   occurrence of a myTriangle on a list can (and does) represent  */
				/*   an edge.  In this way, most edges are represented twice, and */
				/*   every myTriangle-segment bond is represented once.             */
				while (notfound && (checktri.tri != dummytri))
				{
					dest(checktri, checkdest);
					if (shorg == checkdest)
					{
						/* We have a match.  Remove this myTriangle from the list. */
						*prevlink = checktri.tri[6 + checktri.orient];
						/* Bond the shell edge to the myTriangle. */
	...
	for (i = 0; i < points.items; i++)
	{
		/* Search the stack of triangles adjacent to a myPoint. */
		nexttri = vertexarray[i];
		decode(nexttri, checktri);
		while (checktri.tri != dummytri)
		{
			/* Find the next myTriangle in the stack before this */
...
/*****************************************************************************/
/*                                                                           */
/*  finddirection()   Find the first myTriangle on the path from one myPoint     */
/*                    to another.                                            */
/*                                                                           */
/*  Finds the myTriangle that intersects a line segment drawn from the         */
/*  origin of `searchtri' to the myPoint `endpoint', and returns the result    */
/*  in `searchtri'.  The origin of `searchtri' does not change, even though  */
/*  the myTriangle returned may differ from the one passed in.  This routine   */
/*  is used to find the direction to move in to get from one myPoint to        */
/*  another.                                                                 */
/*                                                                           */
/*  The return value notes whether the destination or apex of the found      */
/*  myTriangle is collinear with the two points in question.                   */
/*                                                                           */
/*****************************************************************************/
	...
	myPoint endpoint)
{
	...
	myTriangle ptr;           /* Temporary variable used by onext() and oprev(). */
	...
	if (leftflag && rightflag)
	{
		...
		/*   left or right.  Ask whether it's a myTriangle or a boundary   */
		/*   on the left.                                                */
	...
	while (leftflag)
	{
		...
		if (searchtri->tri == dummytri)
		{
			...
			char error[256];
			sprintf(error, "  myTriangle leading from (%.12g, %.12g) to  (%.12g, %.12g).",
	...
	while (rightflag)
	{
		...
		{
			char error[256];
			sprintf(error, "Internal error in finddirection():  Unable to find a\n"
				"  myTriangle leading from (%.12g, %.12g) to  (%.12g, %.12g).",
				startpoint[0], startpoint[1], endpoint[0], endpoint[1]);
			internalerror(error);
		}
		dest(*searchtri, rightpoint);
...
/*                          a myPoint at the intersection, splitting an        */
/*                          existing shell edge.                             */
/*                                                                           */
...
/*  intersection myPoint as its origin, and endpoint1 as its destination.      */
/*                                                                           */
/*****************************************************************************/
	...
	myPoint endpoint2)
{
	...
	if (denom == 0.0)
	{
		internalerror("Internal error in segmentintersection():  "
			"Attempt to find intersection of parallel segments.");
	}
	split = (ey * etx - ex * ety) / denom;
	/* Create the new myPoint. */
	newpoint = (myPoint)poolalloc(&points);
	...
	/* Insert the intersection myPoint.  This should always succeed. */
	...
	/* Inserting the myPoint may have caused edge flips.  We wish to rediscover */
	/*   the edge connecting endpoint1 to the new intersection myPoint.         */
...
/*****************************************************************************/
/*                                                                           */
/*  scoutsegment()   Scout the first myTriangle on the path from one endpoint  */
/*                   to another, and check for completion (reaching the      */
/*                   second endpoint), a collinear myPoint, and the            */
/*                   intersection of two segments.                           */
/*                                                                           */
/*  Returns one if the entire segment is successfully inserted, and zero if  */
/*  the job must be finished by conformingedge() or constrainededge().       */
/*                                                                           */
/*  If the first myTriangle on the path has the second endpoint as its         */
/*  destination or apex, a shell edge is inserted and the job is done.       */
/*                                                                           */
/*  If the first myTriangle on the path has a destination or apex that lies on */
/*  the segment, a shell edge is inserted connecting the first endpoint to   */
/*  the collinear myPoint, and the search is continued from the collinear      */
/*  myPoint.                                                                   */
/*                                                                           */
/*  If the first myTriangle on the path has a shell edge opposite its origin,  */
/*  then there is a segment that intersects the segment being inserted.      */
/*  Their intersection myPoint is inserted, splitting the shell edge.          */
/*                                                                           */
	...
	int newmark)
{
	...
	{
		/* We've collided with a myPoint between the segment's endpoints. */
		/* Make the collinear myPoint be the myTriangle's origin. */
		lprevself(*searchtri);
		insertshelle(searchtri, newmark);
	...
	else
	{
		...
		else
		{
			...
			/* Insert a myPoint at the intersection. */
...
/*                     triangulation by inserting a myPoint at its midpoint,   */
...
/*  splitting myPoint and shell edge.                                          */
/*                                                                           */
	...
	int newmark)
{
	...
	/* Create a new myPoint to insert in the middle of the segment. */
	newpoint = (myPoint)poolalloc(&points);
	...
	/* Find a boundary myTriangle to search from. */
	searchtri1.tri = (myTriangle*)NULL;
	/* Attempt to insert the new myPoint. */
	success = insertsite(newpoint, &searchtri1, (struct edge*) NULL, 0, 0);
	if (success == DUPLICATEPOINT)
	{
		if (verbose > 2)
		{
			printf("  Segment intersects existing myPoint (%.12g, %.12g).\n",
				newpoint[0], newpoint[1]);
		}
		/* Use the myPoint that's already there. */
	...
	else
	{
		if (success == VIOLATINGPOINT)
		{
			...
			if (success != SUCCESSFULPOINT)
			{
				internalerror("Internal error in conformingedge():\n"
					"  Failure to split a segment.");
			}
		}
		/* The myPoint has been inserted successfully. */
		if (steinerleft > 0)
		{
			steinerleft--;
		}
...
/*                    recursively from an existing myPoint.  Pay special       */
...
/*  the new myPoint (the origin of fixuptri) indicates that the vertex before  */
/*  it on the polygon is a reflex vertex, then "stack" the myTriangle by       */
/*  doing nothing.  (fixuptri is an inverted myTriangle, which is how stacked  */
/*  triangles are identified.)                                               */
/*                                                                           */
/*  Otherwise, check whether the vertex before that was a reflex vertex.     */
/*  If so, perform an edge flip, thereby eliminating an inverted myTriangle    */
/*  (popping it off the stack).  The edge flip may result in the creation    */
/*  of a new inverted myTriangle, depending on whether or not the new vertex   */
/*  is visible to the vertex three edges behind on the polygon.              */
/*                                                                           */
/*  If neither of the two vertices behind the new vertex are reflex          */
/*  vertices, fixuptri and fartri, the myTriangle opposite it, are not         */
	...
	int leftside)
{
	...
	struct edge faredge;
	myPoint nearpoint, leftpoint, rightpoint, farpoint;
	myTriangle ptr;                         /* Temporary variable used by sym(). */
	shelle sptr;                      /* Temporary variable used by tspivot(). */
	...
	{
		/* fartri is not an inverted myTriangle, and farpoint is not a reflex */
		/*   vertex.  As there are no reflex vertices, fixuptri isn't an    */
		/*   inverted myTriangle, either.  Hence, test the edge between the   */
...
/*  myTriangle `starttri' has `endpoint1' as its origin.  `newmark' is the     */
/*  boundary marker of the segment.                                          */
/*                                                                           */
/*  To insert a segment, every myTriangle whose interior intersects the        */
...
/*  polygon, there will be an inverted myTriangle formed by one of the edge    */
/*  flips.  (An inverted myTriangle is one with negative area - that is, its   */
/*  vertices are arranged in clockwise order - and is best thought of as a   */
/*  wrinkle in the fabric of the mesh.)  Each inverted myTriangle can be       */
...
/*  myTriangle will take its place on the stack.)  These details are handled   */
	...
	int newmark)
{
	...
	int collision;
	int done;
	myTriangle ptr;             /* Temporary variable used by sym() and oprev(). */
	shelle sptr;                      /* Temporary variable used by tspivot(). */

	org(*starttri, endpoint1);
	lnext(*starttri, fixuptri);
	flip(&fixuptri);
	/* `collision' indicates whether we have found a myPoint directly */
	...
	do
	{
		...
		/* `farpoint' is the extreme myPoint of the polygon we are "digging" */
		...
		else
		{
			...
			if (area == 0.0)
			{
				/* We've collided with a myPoint between endpoint1 and endpoint2. */
				collision = 1;
				oprev(fixuptri, fixuptri2);
			...
			else
			{
				...
				{
					flip(&fixuptri); /* May create an inverted myTriangle on the left. */
				}
				else
				{
	...
	int newmark)
{
	...
	/* Find a myTriangle whose origin is the segment's first endpoint. */
	checkpoint = (myPoint)NULL;
	encodedtri = point2tri(endpoint1);
	if (encodedtri != (myTriangle)NULL)
	{
	...
	if (checkpoint != endpoint1)
	{
		...
		searchtri1.orient = 0;
		symself(searchtri1);
		/* Search for the segment's first endpoint by myPoint location. */
		if (locate(endpoint1, &searchtri1) != ONVERTEX)
		{
			char error[256];
			sprintf(error,
				"Internal error in insertsegment():  Unable to locate PSLG myPoint\n"
				"  (%.12g, %.12g) in triangulation.", endpoint1[0], endpoint1[1]);
			internalerror(error);
		}
	}
	/* Remember this myTriangle to improve subsequent myPoint location. */
	...
	/* Find a myTriangle whose origin is the segment's second endpoint. */
	checkpoint = (myPoint)NULL;
	...
	if (checkpoint != endpoint2)
	{
		...
		searchtri2.orient = 0;
		symself(searchtri2);
		/* Search for the segment's second endpoint by myPoint location. */
		if (locate(endpoint2, &searchtri2) != ONVERTEX)
		{
			char error[256];
			sprintf(error,
				"Internal error in insertsegment():  Unable to locate PSLG myPoint\n"
				"  (%.12g, %.12g) in triangulation.", endpoint2[0], endpoint2[1]);
			internalerror(error);
		}
	}
	/* Remember this myTriangle to improve subsequent myPoint location. */
...
void markhull()
{
	...
	/* Find a myTriangle handle on the hull. */
...

{
	...
	char* stringptr;
#endif /* not TRILIBRARY */
	myPoint endpoint1, endpoint2;
	int segments, resegments;
	int segmentmarkers;
	int end1, end2;
	int boundmarker;
	int i;
	...
	if (poly)
	{
		...
		/*	 CArray<int,int> newsegmentlist;
			for (i = 1; i <= segments; i++)
			{
				...
				myTriangle encodedtri1 = point2tri(endpoint1);
				myTriangle encodedtri2 = point2tri(endpoint2);
				if(encodetri1 != (myTriangle)0xcdcdcd && encodetri2 != (myTriangle))
				{
					newsegmentlist.Add(end1);
					newsegmentlist.Add(end2);
				}
			}
			if(newsegmentlist.GetSize()!=segments);
			{
			}
		*/
#endif
		...
		for (i = 1; i <= segments; i++)
		{
			...
			if (*stringptr == '\0')
			{
				char error[256];
				sprintf(error,
					"Error:  Segment %d has no endpoints in %s.", i, polyfilename);
				reporterror(error);
				//exit(1);
			}
			else
			...
			if (*stringptr == '\0')
			{
				char error[256];
				sprintf(error,
					"Error:  Segment %d is missing its second endpoint in %s.", i, polyfilename);
				reporterror(error);
				//exit(1);
			}
			else
			...
			else
			{
				...
				{
					//myPoint keyvalue;
					if (g_pTrMap.find(endpoint1) != g_pTrMap.end() && g_pTrMap.find(endpoint2) != g_pTrMap.end())
					{
...
void infecthull()
{
	...
	do
	{
		...
		if (!infected(hulltri))
		{
			/* Is the myTriangle protected by a shell edge? */
			tspivot(hulltri, hulledge);
			if (hulledge.sh == dummysh)
			{
				/* The myTriangle is not protected; infect it. */
				infect(hulltri);
				deadtri = (myTriangle**)poolalloc(&viri);
				*deadtri = hulltri.tri;
			}
			else
			{
				/* The myTriangle is protected; set boundary markers if appropriate. */
				if (mark(hulledge) == 0)
				{
					setmark(hulledge, 1);
					org(hulltri, horg);
...
/*  marked to ensure that each myTriangle is added to the virus pool only      */
...
void plague()
{
	...
	myTriangle** virusloop;
	myTriangle** deadtri;
	struct edge neighborshelle;
	myPoint testpoint;
	myPoint norg, ndest;
	myPoint deadorg, deaddest, deadapex;
	int killorg;
	myTriangle ptr;             /* Temporary variable used by sym() and onext(). */
	shelle sptr;                      /* Temporary variable used by tspivot(). */
	...
	traversalinit(&viri);
	virusloop = (myTriangle**)traverse(&viri);
	while (virusloop != (myTriangle**)NULL)
	{
		testtri.tri = *virusloop;
		/* A myTriangle is marked as infected by messing with one of its shell */
		/*   edges, setting it to an illegal value.  Hence, we have to       */
		/*   temporarily uninfect this myTriangle so that we can examine its   */
		/*   adjacent shell edges.                                           */
		uninfect(testtri);
		if (verbose > 2)
		{
			/* Assign the myTriangle an orientation for convenience in */
			/*   checking its points.                                */
			testtri.orient = 0;
			org(testtri, deadorg);
		...
		/* Check each of the myTriangle's three neighbors. */
		for (testtri.orient = 0; testtri.orient < 3; testtri.orient++)
		{
			/* Find the neighbor. */
			sym(testtri, neighbor);
			/* Check for a shell between the myTriangle and its neighbor. */
			...
			if ((neighbor.tri == dummytri) || infected(neighbor))
			{
				...
				{
					/* There is a shell edge separating the myTriangle from its */
			...
			else                       /* The neighbor exists and is not infected. */
			{
				...
				{
					/* Remove this myTriangle from the shell edge. */
		...
		/* Remark the myTriangle as infected, so it doesn't get added to the */
		/*   virus pool again.                                             */
		infect(testtri);
		virusloop = (myTriangle**)traverse(&viri);
	}
	...
	while (virusloop != (myTriangle**)NULL)
	{
		...
		/* Check each of the three corners of the myTriangle for elimination. */
		/*   This is done by walking around each myPoint, checking if it is   */
		/*   still connected to at least one live myTriangle.                 */
		for (testtri.orient = 0; testtri.orient < 3; testtri.orient++)
		{
			org(testtri, testpoint);
			/* Check if the myPoint has already been tested. */
			if (testpoint != (myPoint)NULL)
			{
				killorg = 1;
				/* Mark the corner of the myTriangle as having been tested. */
				setorg(testtri, NULL);
				/* Walk counterclockwise about the myPoint. */
				onext(testtri, neighbor);
				/* Stop upon reaching a boundary or the starting myTriangle. */
				while ((neighbor.tri != dummytri)
					&& (!triedgeequal(neighbor, testtri)))
				{
					if (infected(neighbor))
					{
						/* Mark the corner of this myTriangle as having been tested. */
						setorg(neighbor, NULL);
					}
					else
					{
						/* A live myTriangle.  The myPoint survives. */
						killorg = 0;
					}
					/* Walk counterclockwise about the myPoint. */
					onextself(neighbor);
				...
				if (neighbor.tri == dummytri)
				{
					/* Walk clockwise about the myPoint. */
				...
				if (killorg)
				{
					if (verbose > 1)
					{
						printf("    Deleting myPoint (%.12g, %.12g)\n",
		...
		for (testtri.orient = 0; testtri.orient < 3; testtri.orient++)
		{
			...
			if (neighbor.tri == dummytri)
			{
				/* There is no neighboring myTriangle on this edge, so this edge    */
				/*   is a boundary edge.  This myTriangle is being deleted, so this */
				/*   boundary edge is deleted.                                    */
				hullsize--;
			}
			else
			{
				/* Disconnect the myTriangle from its neighbor. */
				dissolve(neighbor);
				/* There is a neighboring myTriangle on this edge, so this edge */
				/*   becomes a boundary edge when this myTriangle is deleted.   */
				hullsize++;
			}
		}
		/* Return the dead myTriangle to the pool of triangles. */
...
/*  The triangles are marked to ensure that each myTriangle is added to the    */
	...
	REAL area)
{
	...
	struct triedge neighbor;
	myTriangle** virusloop;
	myTriangle** regiontri;
	struct edge neighborshelle;
	myPoint regionorg, regiondest, regionapex;
	myTriangle ptr;             /* Temporary variable used by sym() and onext(). */
	shelle sptr;                      /* Temporary variable used by tspivot(). */
	...
	while (virusloop != (myTriangle**)NULL)
	{
		...
		for (testtri.orient = 0; testtri.orient < 3; testtri.orient++)
		{
				...
				&& (neighborshelle.sh == dummysh))
			{
				...
				infect(neighbor);
				/* Ensure that the neighbor's neighbors will be infected. */
				regiontri = (myTriangle**)poolalloc(&viri);
	...
	int regions)
{
	...
	if (((holes > 0) && !noholes) || !convex || (regions > 0))
	{
		...
		poolinit(&viri, sizeof(myTriangle*), VIRUSPERBLOCK, POINTER, 0);
	}
	...
	if ((holes > 0) && !noholes)
	{
		/* Infect each myTriangle in which a hole lies. */
		for (i = 0; i < 2 * holes; i += 2)
		{
			...
			{
				/* Start searching from some myTriangle on the outer boundary. */
				searchtri.tri = dummytri;
				searchtri.orient = 0;
				symself(searchtri);
				/* Ensure that the hole is to the left of this boundary edge; */
				/*   otherwise, locate() will falsely report that the hole    */
				/*   falls within the starting myTriangle.                      */
				org(searchtri, searchorg);
				dest(searchtri, searchdest);
				if (counterclockwise(searchorg, searchdest, &holelist[i]) > 0.0)
				{
					/* Find a myTriangle that contains the hole. */
					intersect = locate(&holelist[i], &searchtri);
					if ((intersect != OUTSIDE) && (!infected(searchtri)))
					{
						/* Infect the myTriangle.  This is done by marking the myTriangle */
						/*   as infect and including the myTriangle in the virus pool.  */
						infect(searchtri);
						holetri = (myTriangle**)poolalloc(&viri);
	...
	if (regions > 0)
	{
		/* Find the starting myTriangle for each region. */
		for (i = 0; i < regions; i++)
		{
				...
				(regionlist[4 * i + 1] >= ymin) && (regionlist[4 * i + 1] <= ymax))
			{
				...
				searchtri.orient = 0;
				symself(searchtri);
				/* Ensure that the region myPoint is to the left of this boundary */
				/*   edge; otherwise, locate() will falsely report that the     */
				/*   region myPoint falls within the starting myTriangle.           */
				org(searchtri, searchorg);
				dest(searchtri, searchdest);
				if (counterclockwise(searchorg, searchdest, &regionlist[4 * i]) >
					0.0)
				{
					/* Find a myTriangle that contains the region myPoint. */
					intersect = locate(&regionlist[4 * i], &searchtri);
					if ((intersect != OUTSIDE) && (!infected(searchtri)))
					{
						/* Record the myTriangle for processing after the */
	...
	if (regions > 0)
	{
		...
		{
			/* Assign every myTriangle a regional attribute of zero. */
			traversalinit(&triangles);
			triangleloop.orient = 0;
			triangleloop.tri = triangletraverse();
			while (triangleloop.tri != (myTriangle*)NULL)
		...
		for (i = 0; i < regions; i++)
		{
			...
			{
				/* Make sure the myTriangle under consideration still exists. */
				/*   It may have been eaten by the virus.                   */
				if (regiontris[i].tri[3] != (myTriangle)NULL)
				{
					/* Put one myTriangle in the virus pool. */
					infect(regiontris[i]);
					regiontri = (myTriangle**)poolalloc(&viri);
		...
		if (regionattrib && !refine)
		{
			/* Note the fact that each myTriangle has an additional attribute. */
			eextras++;
		}
	}
...
void precisionerror(const char* lpszerror)
{
	char error[1024];
	sprintf(error, "%s\n%s", lpszerror,
...
/*                                                                           */
/*  Encroached segments are repaired by splitting them by inserting a myPoint  */
	...
	int flaws)
{
	...
	while ((badsegments.items > 0) && (steinerleft != 0))
	{
		...
		while ((encloop != (struct edge*) NULL) && (steinerleft != 0))
		{
			...
			/*   myPoint added to split one segment will encroach upon the   */
			/*   other segment, which must then be split with a myPoint that */
			...
			/* Now, check the other side of the segment, if there's a myTriangle */
			/*   there.                                                        */
			sym(enctri, testtri);
			if (testtri.tri != dummytri)
			{
			...
			/* Check whether the new myPoint lies on an endpoint. */
				...
				|| ((newpoint[0] == edest[0]) && (newpoint[1] == edest[1])))
			{
				...
				sprintf(error,
					...
					"  be accommodated by the finite precision of floating myPoint\n"
					"  arithmetic.", newpoint[0], newpoint[1]);
				precisionerror(error);
			}
			/* Insert the splitting myPoint.  This should always succeed. */
			success = insertsite(newpoint, &enctri, encloop, flaws, flaws);
			if ((success != SUCCESSFULPOINT) && (success != ENCROACHINGPOINT))
			{
				internalerror("Internal error in repairencs():\n"
					"  Failure to split a segment.");
			}
			if (steinerleft > 0)
			{
...
/*****************************************************************************/
/*                                                                           */
/*  tallyfaces()   Test every myTriangle in the mesh for quality measures.     */
/*                                                                           */
/*****************************************************************************/
...
void tallyfaces()
{
	...
	while (triangleloop.tri != (myTriangle*)NULL)
	{
		/* If the myTriangle is bad, enqueue it. */
...
/*****************************************************************************/
/*                                                                           */
/*  findcircumcenter()   Find the circumcenter of a myTriangle.                */
/*                                                                           */
/*  The result is returned both in terms of x-y coordinates and xi-eta       */
/*  coordinates.  The xi-eta coordinate system is defined in terms of the    */
/*  myTriangle:  the origin of the myTriangle is the origin of the coordinate    */
/*  system; the destination of the myTriangle is one unit along the xi axis;   */
/*  and the apex of the myTriangle is one unit along the eta axis.             */
/*                                                                           */
/*  The return value indicates which edge of the myTriangle is shortest.       */
/*                                                                           */
/*****************************************************************************/
	...
	REAL* eta)
{
	...
	/* Compute the circumcenter of the myTriangle. */
	xdo = tdest[0] - torg[0];
	ydo = tdest[1] - torg[1];
	xao = tapex[0] - torg[0];
	yao = tapex[1] - torg[1];
	...
	/* To interpolate myPoint attributes for the new myPoint inserted at  */
	/*   the circumcenter, define a coordinate system with a xi-axis, */
	/*   directed from the myTriangle's origin to its destination, and  */
...
/*****************************************************************************/
/*                                                                           */
/*  splittriangle()   Inserts a myPoint at the circumcenter of a myTriangle.     */
/*                    Deletes the newly inserted myPoint if it encroaches upon */
/*                    a segment.                                             */
/*                                                                           */
/*****************************************************************************/
	...
	struct badface* badtri)
{
	...
	/* Make sure that this myTriangle is still the same myTriangle it was      */
	/*   when it was tested and determined to be of bad quality.           */
	/*   Subsequent transformations may have made it a different myTriangle. */
	if ((borg == badtri->faceorg) && (bdest == badtri->facedest) &&
		(bapex == badtri->faceapex))
	{
		if (verbose > 1)
		{
			printf("  Splitting this myTriangle at its circumcenter:\n");
			printf("    (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n", borg[0],
				borg[1], bdest[0], bdest[1], bapex[0], bapex[1]);
		}
		errorflag = 0;
		/* Create a new myPoint at the myTriangle's circumcenter. */
		newpoint = (myPoint)poolalloc(&points);
		shortedge = findcircumcenter(borg, bdest, bapex, newpoint, &xi, &eta);
		/* Check whether the new myPoint lies on a myTriangle vertex. */
			...
			|| ((newpoint[0] == bapex[0]) && (newpoint[1] == bapex[1])))
		{
			if (!quiet)
			{
				printf("Warning:  New myPoint (%.12g, %.12g) falls on existing vertex.\n"
					, newpoint[0], newpoint[1]);
				errorflag = 1;
			}
			pointdealloc(newpoint);
		...
		else
		{
			...
			{
				/* Interpolate the myPoint attributes at the circumcenter. */
				newpoint[i] = borg[i] + xi * (bdest[i] - borg[i])
					+ eta * (bapex[i] - borg[i]);
			}
			/* The new myPoint must be in the interior, and have a marker of zero. */
			setpointmark(newpoint, 0);
			/* Ensure that the handle `badtri->badfacetri' represents the shortest */
			/*   edge of the myTriangle.  This ensures that the circumcenter must    */
			/*   fall to the left of this edge, so myPoint location will work.       */
			...
			/* Insert the circumcenter, searching from the edge of the myTriangle, */
			...
			{
				/* If the newly inserted myPoint encroaches upon a segment, delete it. */
				deletesite(&(badtri->badfacetri));
			}
			else if (success == VIOLATINGPOINT)
			{
				/* Failed to insert the new myPoint, but some segment was */
				/*   marked as being encroached.                        */
				pointdealloc(newpoint);
			}
			else                                      /* success == DUPLICATEPOINT */
			{
				/* Failed to insert the new myPoint because a vertex is already there. */
				if (!quiet)
				{
					printf(
						"Warning:  New myPoint (%.12g, %.12g) falls on existing vertex.\n"
						, newpoint[0], newpoint[1]);
					errorflag = 1;
				}
				pointdealloc(newpoint);
		...
		if (errorflag)
		{
			if (verbose)
			{
				printf("  The new myPoint is at the circumcenter of myTriangle\n");
				printf("    (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n",
					borg[0], borg[1], bdest[0], bdest[1], bapex[0], bapex[1]);
			}
			printf("This probably means that I am trying to refine triangles\n");
			printf("  to a smaller size than can be accommodated by the finite\n");
			printf("  precision of floating myPoint arithmetic.  (You can be\n");
			printf("  sure of this if I fail to terminate.)\n");
			precisionerror();
		}
	}
	/* Return the bad myTriangle to the pool. */
...
void enforcequality()
{
	...
	/* At this myPoint, if we haven't run out of Steiner points, the */
	/*   triangulation should be (conforming) Delaunay.            */

	/* Next, we worry about enforcing myTriangle quality. */
	if ((minangle > 0.0) || vararea || fixedarea)
	{
		...
		{
			/* Fix one bad myTriangle by inserting a myPoint at its circumcenter. */
...
void highorder()
{
	...
	/*   the three edges of each myTriangle.  If there isn't another myTriangle  */
	/*   adjacent to the edge, operate on the edge.  If there is another     */
	/*   adjacent myTriangle, operate on the edge only if the current myTriangle */
	...
	while (triangleloop.tri != (myTriangle*)NULL)
	{
			...
			triangleloop.orient++)
		{
			...
			if ((triangleloop.tri < trisym.tri) || (trisym.tri == dummytri))
			{
				...
				{
					trisym.tri[highorderindex + trisym.orient] = (myTriangle)newpoint;
				}
			}
		}
		triangleloop.tri = triangletraverse();
	...
	FILE** polyfile)
{
	...
	if (poly)
	{
		...
		*polyfile = fopen(polyfilename, "r");
		...
		/* Read number of points, number of dimensions, number of myPoint */
	...
	if (inpoints < 3)
	{
		reporterror("Error:  Input must have at least three input points.");
		//exit(1);
	}
	if (mesh_dim != 2)
	{
		reporterror("Error:  Triangle only works with two-dimensional meshes.");
		//exit(1);
	}
	...
	for (i = 0; i < inpoints; i++)
	{
		pointloop = (myPoint)poolalloc(&points);
		...
		if (*stringptr == '\0')
		{
			char error[256];
			sprintf(error,
				"Error:  Point %d has no x coordinate.", firstnumber + i);
			reporterror(error);
			//exit(1);
		}
		...
		if (*stringptr == '\0')
		{
			char error[256];
			sprintf(error,
				"Error:  Point %d has no y coordinate.", firstnumber + i);
			reporterror(error);
			//exit(1);
		}
		...
		*hlist = holelist;
		...
		for (i = 0; i < 2 * *holes; i += 2)
		{
			...
			if (*stringptr == '\0')
			{
				char error[256];
				sprintf(error,
					"Error:  Hole %d has no x coordinate.", firstnumber + (i >> 1));
				reporterror(error);
				//exit(1);
			}
			else
			...
			if (*stringptr == '\0')
			{
				char error[256];
				sprintf(error,
					("Error:  Hole %d has no y coordinate.", firstnumber + (i >> 1));
				reporterror(error);
				//exit(1);
			}
			else
			...
			*rlist = regionlist;
			...
			for (i = 0; i < *regions; i++)
			{
				...
				if (*stringptr == '\0')
				{
					char error[256];
					sprintf(error,
						"Error:  Region %d has no x coordinate.", firstnumber + i);
					reporterror(error);
					//exit(1);
				}
				else
				...
				if (*stringptr == '\0')
				{
					char error[256];
					sprintf(error,
						"Error:  Region %d has no y coordinate.", firstnumber + i);
					reporterror(error);
					//exit(1);
				}
				else
...
/*                                                                           */
/*  To save memory, the myPoint numbers are written over the shell markers     */
...

{
	...
	/* Allocate memory for output myPoint attributes if necessary. */
	...
	/* Allocate memory for output myPoint markers if necessary. */
	...
	if (outfile == (FILE*)NULL)
	{
		char error[256];
		sprintf(error,
			"  Error:  Cannot create file %s.", nodefilename);
		reporterror(error);
		//exit(1);
	}
	/* Number of points, number of dimensions, number of myPoint attributes, */
	...
	int lNum = (long)sqrt((float)g_nProcessed);
	if (lNum > 100) { lNum = 100; }
	float ratio = (float)g_nProcessed / lNum;
	//  long nProgress = 0;
	//  if(g_triprogress)g_triprogress->ShowMessage(_bstr_t("Writing points"));
	// if(g_triprogress)g_triprogress->StartProgress(lNum);
	while (pointloop != (myPoint)NULL)
	{
#ifdef TRILIBRARY
		...
		pointloop = pointtraverse();
		//	if((long)(pointnumber/ratio)>nProgress){
		//		if(g_triprogress)g_triprogress->StepIt();
		//		nProgress++;
		//	}
		pointnumber++;
...
/*                                                                           */
/*  Each myPoint is assigned a marker equal to its number.                     */
/*                                                                           */
...

{
	...
	/* Allocate memory for output myTriangle attributes if necessary. */
	...
	if (outfile == (FILE*)NULL)
	{
		char error[256];
		sprintf(error,
			"  Error:  Cannot create file %s.", elefilename);
		reporterror(error);
		//exit(1);
	}
	/* Number of triangles, points per myTriangle, attributes per myTriangle. */
	...
	triangleloop.orient = 0;
	elementnumber = firstnumber;
	int lNum = (long)sqrt((float)g_nProcessed * 2);
	if (lNum > 100) { lNum = 100; }
	float ratio = (float)g_nProcessed * 2 / lNum;
	//  long nProgress = 0;
	//  if(g_triprogress)g_triprogress->ShowMessage(_bstr_t("Writing tris"));
	//  if(g_triprogress)g_triprogress->StartProgress(lNum);
	while (triangleloop.tri != (myTriangle*)NULL)
	{
		org(triangleloop, p1);
		...
		{
			mid1 = (myPoint)triangleloop.tri[highorderindex + 1];
			mid2 = (myPoint)triangleloop.tri[highorderindex + 2];
			mid3 = (myPoint)triangleloop.tri[highorderindex];
#ifdef TRILIBRARY
			tlist[pointindex++] = pointmark(p1);
			tlist[pointindex++] = pointmark(p2);
			tlist[pointindex++] = pointmark(p3);
		...
		triangleloop.tri = triangletraverse();
		//	if((long)(elementnumber/ratio)>nProgress){
		//		if(g_triprogress)g_triprogress->StepIt();
		//		nProgress++;
		//	}
		elementnumber++;
...

{
	...
	if (outfile == (FILE*)NULL)
	{
		char error[256];
		sprintf(error,
			"  Error:  Cannot create file %s.", polyfilename);
		reporterror(error);
		//exit(1);
	}
	/* The zero indicates that the points are in a separate .node file. */
	/*   Followed by number of dimensions, number of myPoint attributes,  */
	/*   and number of boundary markers (zero or one).                  */
...

{
	...
	if (outfile == (FILE*)NULL)
	{
		char error[256];
		sprintf(error,
			"  Error:  Cannot create file %s.", edgefilename);
		reporterror(error);
		//exit(1);
	}
...

{
	...
	*vpointmarkerlist = (int*)NULL;
	...
	/* Number of triangles, two dimensions, number of myPoint attributes, */
	/*   zero markers.                                                  */
	...
	*vedgemarkerlist = (int*)NULL;
	...
	if (outfile == (FILE*)NULL)
	{
		char error[256];
		sprintf(error,
			"  Error:  Cannot create file %s.", vedgefilename);
		reporterror(error);
		//exit(1);
	}
	...
	while (triangleloop.tri != (myTriangle*)NULL)
	{
			...
			triangleloop.orient++)
		{
			...
			if ((triangleloop.tri < trisym.tri) || (trisym.tri == dummytri))
			{
				/* Find the number of this myTriangle (and Voronoi vertex). */
				...
				{
					/* Find the number of the adjacent myTriangle (and Voronoi vertex). */
					p2 = *(int*)(trisym.tri + 6);
...

{
	...
	if (!quiet)
	{
		char error[256];
		sprintf(error,
			"  Error:  Cannot create file %s.", neighborfilename);
		reporterror(error);
	}
	...
	/* Number of triangles, three edges per myTriangle. */
	fprintf(outfile, "%ld  %d\n", triangles.items, 3);
#endif /* not TRILIBRARY */
...
#else /* not TRILIBRARY */
		/* Triangle number, neighboring myTriangle numbers. */
		fprintf(outfile, "%4d    %d  %d  %d\n", elementnumber,
	...
	char** argv)
{
	...
	if (outfile == (FILE*)NULL)
	{
		char error[256];
		sprintf(error,
			"  Error:  Cannot create file %s.", offfilename);
		reporterror(error);
		//exit(1);
	}
...

{
	...
	/* Ensure that no myPoint can be mistaken for a triangular bounding */
	/*   box myPoint in insertsite().                                   */
	infpoint1 = (myPoint)NULL;
	infpoint2 = (myPoint)NULL;
	infpoint3 = (myPoint)NULL;
	...
	if (poly)
	{
		...
		holearray = (REAL*)malloc(2 * holes * sizeof(REAL));
		memcpy(holearray, in->holelist, 2 * holes * sizeof(REAL));
		//regionarray = in->regionlist;
		regionarray = (REAL*)malloc(4 * regions * sizeof(REAL));
		memcpy(regionarray, in->regionlist, 4 * regions * sizeof(REAL));
	...
	if (poly || convex)
	{
		...
		else
		{
			...
			if (poly)
			{
				...
				out->holelist = (REAL*)malloc(2 * in->numberofholes * sizeof(REAL));
				memcpy(out->holelist, in->holelist, 2 * in->numberofholes * sizeof(REAL));
				//out->regionlist = in->regionlist;
				out->regionlist = (REAL*)malloc(4 * in->numberofregions * sizeof(REAL));
				memcpy(out->regionlist, in->regionlist, 4 * in->numberofregions * sizeof(REAL));
			}
			else
			{
...
TriangleX::TriangleX()
{
	...
	memset(m_vorout, 0, sizeof(triangulateio));

	/*	g_pTrMap.InitHashTable(1031);*/
}
...
void TriangleX::Clear(struct triangulateio* t)
{
	if (t->holelist)
	{
		delete t->holelist;
...
struct triangulateio* TriangleX::GetOut()
{
	return m_out;
}

struct triangulateio* TriangleX::GetVorout()
{
	return m_vorout;
}

void TriangleX::ResetContent()
{
	delete m_out;
	delete m_vorout;
	m_out = new struct triangulateio;
...
void TriangleX::Build(char* triswitches, struct triangulateio* in)
{
	try
	{
		ResetContent();
		triangulate(triswitches, in, m_out, m_vorout);
	}
	catch (...)
	{
	}
