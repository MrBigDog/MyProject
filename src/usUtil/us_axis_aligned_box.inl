...
axis_aligned_box<real>::axis_aligned_box(vector3<real>* verts, int count)
{
	...
	{
		m_min_vector.x = min(m_min_vector.x, verts[ni].x);
		m_min_vector.y = min(m_min_vector.y, verts[ni].y);
		m_min_vector.z = min(m_min_vector.z, verts[ni].z);

		m_max_vector.x = max(m_max_vector.x, verts[ni].x);
		m_max_vector.y = max(m_max_vector.y, verts[ni].y);
		m_max_vector.z = max(m_max_vector.z, verts[ni].z);
	}
	update_corners();
}
