	...
	edge_array& d3d9_hardware_mesh_buffer::get_edge(void)
	{
		...
		if (m_32bit_index)
		{
			...
			for (int i = 0; i < (int)m_num_of_triangles; i++)
			{
				...
				// the first line
				v_pair1 = std::make_pair(v1, v2);
				itr = v_line_map.find(v_pair1);
				if (itr == v_line_map.end())
				{
					v_pair2 = std::make_pair(v2, v1);
				...
				// the second line
				v_pair1 = std::make_pair(v2, v3);
				itr = v_line_map.find(v_pair1);
				if (itr == v_line_map.end())
				{
					v_pair2 = std::make_pair(v3, v2);
					itr = v_line_map.find(v_pair2);
					if (itr == v_line_map.end())
					{
						edge v_line;
				...
				// the third line
				v_pair1 = std::make_pair(v3, v1);
				itr = v_line_map.find(v_pair1);
				if (itr == v_line_map.end())
				{
					v_pair2 = std::make_pair(v1, v3);
					itr = v_line_map.find(v_pair2);
					if (itr == v_line_map.end())
					{
						edge v_line;
