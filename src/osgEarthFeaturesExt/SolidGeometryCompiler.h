#ifndef OEFEATURESEXT_SOLIDGEOMETRYCOMPILER_H
#define OEFEATURESEXT_SOLIDGEOMETRYCOMPILER_H 1
...
{
	class OSGEARTHFEATRUESEXT_EXPORT SolidGeometryCompilerOptions : public osgEarth::ConfigOptions
	{
	public:
		/* Set the global default values for the options. */
		static void setDefaults(const SolidGeometryCompilerOptions& defaults);

	public:
		SolidGeometryCompilerOptions(const ConfigOptions& conf = ConfigOptions());
		virtual ~SolidGeometryCompilerOptions() { }
	...
	private:
		...
		static SolidGeometryCompilerOptions s_defaults;

	public:
		SolidGeometryCompilerOptions(bool); // internal
	};


	class SolidGeometryCompiler
	{
	protected:
		SolidGeometryCompilerOptions options_;

	public:
		SolidGeometryCompiler(void);
		SolidGeometryCompiler(const SolidGeometryCompilerOptions& options);

		~SolidGeometryCompiler(void);

		//Ö»¶Á;
		const SolidGeometryCompilerOptions& options() const
		{
			return options_;
		}

		//¿É±à¼­;
		SolidGeometryCompilerOptions& options()
		{
			return options_;
		}
...
#endif // GWPIPELINE_SolidGeometryCompiler_H
