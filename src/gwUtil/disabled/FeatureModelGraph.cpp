#include <gwUtil/FeatureModelGraph.h>
...
#define LC "[FeatureModelGraph1] " << getName() << ": "
...
namespace gwUtil
{
	...
	// pseudo-loader for paging in feature tiles for a FeatureModelGraph1.
	namespace
	{
		UID                               _uid = 0;
		Threading::ReadWriteMutex         _fmgMutex;
		typedef std::map<UID, osg::observer_ptr<FeatureModelGraph1> > FMGRegistry;
		FMGRegistry _fmgRegistry;

		static std::string s_makeURI(UID uid, unsigned lod, unsigned x, unsigned y)
		{
			std::stringstream buf;
			buf << uid << "." << lod << "_" << x << "_" << y << ".osgearth_pseudo_fmg1";
			std::string str;
			str = buf.str();
			return str;
		}

		class MyComputeRangeCallback :public TilePagedLOD::ComputeRangeCallback
		{
		public:
			MyComputeRangeCallback(float maxrange) {}
			virtual float operator()(osg::Node* node, osg::NodeVisitor& nv)
			{
				...
				float res = osg::maximum(0.1f, distance - radius);
		...
		{
			TilePagedLODWithNodeOperations* p = new TilePagedLODWithNodeOperations(postMergeOps);
			...
			//p->setComputeRangeCallback(new MyComputeRangeCallback(maxRange));

			//GeoPoint centroid;
			////model->_tileKey.getExtent().getCentroid(centroid);
			//osg::Vec3d centerModel;
			//centroid.toWorld(centerModel);
			...
			//osg::BoundingBoxd bb;
			//p->setChildBoundingBoxAndMatrix(1, bb, local2world);
	...
	struct osgEarthFeatureModelPseudoLoader : public osgDB::ReaderWriter
	{
		...
		{
			supportsExtension("osgearth_pseudo_fmg1", "Feature model pseudo-loader");
		}
		...
		ReadResult readNode(const std::string& uri, const osgDB::Options* readOptions) const
		{
			...
			osg::ref_ptr<FeatureModelGraph1> graph = getGraph(uid);
			if (graph.valid())
			{
		...
		static UID registerGraph(FeatureModelGraph1* graph)
		{
		...
		static FeatureModelGraph1* getGraph(UID uid)
		{
	...
	REGISTER_OSGPLUGIN(osgearth_pseudo_fmg1, osgEarthFeatureModelPseudoLoader);
	...
	FeatureModelGraph1::FeatureModelGraph1(Session*                         session,
	...
	FeatureModelGraph1::~FeatureModelGraph1()
	{
		osgEarthFeatureModelPseudoLoader::unregisterGraph(_uid);
	}

	void FeatureModelGraph1::dirty()
	{
		_dirty = true;
	}
	...
	osg::BoundingSphered FeatureModelGraph1::getBoundInWorldCoords(const GeoExtent& extent, const MapFrame*  mapf) const
	{
	...
	osg::Node* FeatureModelGraph1::setupPaging()
	{
	 ...
	 */
	osg::Node* FeatureModelGraph1::load(unsigned lod, unsigned tileX, unsigned tileY,
		const std::string& uri,
		const osgDB::Options* readOptions)
	{
		OE_DEBUG << LC
		...
		if (_useTiledSource)
		{
			...
			// The tilekey x, y, z that is computed in the FeatureModelGraph1 uses a lower left origin,
	...
	void FeatureModelGraph1::buildSubTilePagedLODs(unsigned        parentLOD,
	...
	osg::Group* FeatureModelGraph1::readTileFromCache(const std::string&    cacheKey,
		const osgDB::Options* readOptions)
	{
	...
	bool FeatureModelGraph1::writeTileToCache(const std::string&    cacheKey,
		osg::Group*           node,
	 ...
	 * data source.
	 */
	osg::Group* FeatureModelGraph1::buildTile(const FeatureLevel& level,
		const GeoExtent& extent,
		const TileKey* key,
		const osgDB::Options* readOptions)
	{
	...
	osg::Group* FeatureModelGraph1::build(const Style&          defaultStyle,
	...
	bool FeatureModelGraph1::createOrUpdateNode(FeatureCursor*           cursor,
		const Style&             style,
	 ...
	 */
	void
		FeatureModelGraph1::buildStyleGroups(const StyleSelector*  selector,
			const Query&          baseQuery,
			FeatureIndexBuilder*  index,
			osg::Group*           parent,
			const osgDB::Options* readOptions)
	 ...
	 */
	void FeatureModelGraph1::queryAndSortIntoStyleGroups(const Query&            query,
	...
	osg::Group* FeatureModelGraph1::createStyleGroup(const Style&          style,
	...
	void FeatureModelGraph1::checkForGlobalStyles(const Style& style)
	{
	...
	void FeatureModelGraph1::applyRenderSymbology(const Style& style, osg::Node* node)
	{
		const RenderSymbol* render = style.get<RenderSymbol>();
		if (render && node)
		{
	...
	osg::Group* FeatureModelGraph1::getOrCreateStyleGroupFromFactory(const Style& style)
	{
	...
	void
		FeatureModelGraph1::traverse(osg::NodeVisitor& nv)
	{
	...
	void FeatureModelGraph1::runPreMergeOperations(osg::Node* node)
	{
		if (_preMergeOperations.valid())
		{
	...
	void FeatureModelGraph1::runPostMergeOperations(osg::Node* node)
	{
		if (_postMergeOperations.valid())
		{
	...
	void FeatureModelGraph1::changeOverlay()
	{
		OpenThreads::ScopedLock< OpenThreads::ReentrantMutex > lk(_clampableMutex);
	...
	void FeatureModelGraph1::redraw()
	{
		OpenThreads::ScopedLock< OpenThreads::ReentrantMutex > lk(_clampableMutex);
	...
	void FeatureModelGraph1::setStyles(StyleSheet* styles)
	{
