...
#define   ALIGN16( x ) __declspec(align(16)) x
	...
	void USFILTER_EXPORT CompressImageDXT1(const byte *inBuf, byte *outBuf, int width, int height, int &outputBytes);

	// Compress to DXT5 format
	void USFILTER_EXPORT CompressImageDXT5(const byte *inBuf, byte *outBuf, int width, int height, int &outputBytes);

	// Compress to DXT5 format, first convert to YCoCg color space
	void USFILTER_EXPORT CompressImageDXT5YCoCg(const byte *inBuf, byte *outBuf, int width, int height, int &outputBytes);

	// Compute error between two images
	double USFILTER_EXPORT ComputeError(const byte *original, const byte *dxt, int width, int height);
