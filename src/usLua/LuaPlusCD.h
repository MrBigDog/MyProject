...
#ifdef WIN64
	inline void Push(lua_State* L, INT_PTR value)				
	{ 
		lua_pushinteger(L, value); 
	}
	inline bool	Match(TypeWrapper<INT_PTR>, lua_State* L, int idx)
	{ 
		return lua_type(L, idx) == LUA_TNUMBER; 
	}

	inline INT_PTR				Get(TypeWrapper<INT_PTR>, lua_State* L, int idx)
	{ 
		return static_cast<INT_PTR>(lua_tointeger(L, idx)); 
	}
#endif // WIN64
