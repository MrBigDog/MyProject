#ifndef GWPIPELINE_BUILDPIPELINEGEOMETRYFILTER_H
#define GWPIPELINE_BUILDPIPELINEGEOMETRYFILTER_H 1
...
{
	class BuildPipelineGeometryFilter : public FeaturesToNodeFilter
	{
	public:
		BuildPipelineGeometryFilter( const Style& style =Style() );
		virtual ~BuildPipelineGeometryFilter() { delete _eleQuery; }
	...
	protected:
		...
		osg::Group* processLines           (FeatureList& input, const InstanceSymbol* symbol, FilterContext& cx);
		osg::Geode* processPoints          (FeatureList& input, const InstanceSymbol* symbol, FilterContext& cx);
		osg::Geode* processJoints(FeatureList& input, const InstanceSymbol* symbol, FilterContext& cx);
		osg::Geode* processEquipments(FeatureList& input, const InstanceSymbol* symbol, FilterContext& cx);
		//osg::Geode* 

		//osg::Geode* processPipelines(FeatureList& input, FilterContext& cx);
		osg::Geode* processPipejoints(FeatureList& input, FilterContext& cx);
		osg::Geode* processPipeequipments(FeatureList& input, FilterContext& cx);
...
#endif // GWPIPELINE_BUILDPIPELINEGEOMETRYFILTER_H
