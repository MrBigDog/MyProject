...
{
	PipelineGeometryCompilerOptions PipelineGeometryCompilerOptions::s_defaults(true);

	void PipelineGeometryCompilerOptions::setDefaults(const PipelineGeometryCompilerOptions& defaults)
	{
		s_defaults = defaults;
	}

	// defaults.
	PipelineGeometryCompilerOptions::PipelineGeometryCompilerOptions(bool stockDefaults) 
	...
	//-----------------------------------------------------------------------
	PipelineGeometryCompilerOptions::PipelineGeometryCompilerOptions(const ConfigOptions& conf) 
		: ConfigOptions          ( conf )
	...
	void PipelineGeometryCompilerOptions::fromConfig( const Config& conf )
	{
	...
	Config PipelineGeometryCompilerOptions::getConfig() const
	{
	...
	void PipelineGeometryCompilerOptions::mergeConfig( const Config& conf )
	{
	...
	PipelineGeometryCompiler::PipelineGeometryCompiler()
	{
		//nop
	}

	PipelineGeometryCompiler::PipelineGeometryCompiler( const PipelineGeometryCompilerOptions& options )
		: options_( options )
	{
		//nop
	}

	PipelineGeometryCompiler::~PipelineGeometryCompiler(void)
	{

	}

	osg::Node* PipelineGeometryCompiler::compile(Geometry* geometry, const Style& style, const FilterContext& context)
	{
		osg::ref_ptr<Feature> f = new Feature(geometry, 0L); // no SRS!
		return compile(f.get(), style, context);
	}

	osg::Node* PipelineGeometryCompiler::compile(Geometry* geometry, const Style& style)
	{
		osg::ref_ptr<Feature> f = new Feature(geometry, 0L); // no SRS!
		return compile(f.get(), style, FilterContext(0L) );
	}

	osg::Node* PipelineGeometryCompiler::compile(Geometry* geometry, const FilterContext&  context)
	{
		return compile( geometry, Style(), context );
	}

	osg::Node* PipelineGeometryCompiler::compile(Feature* feature, const Style& style, const FilterContext& context)
	{
	...
	osg::Node* PipelineGeometryCompiler::compile(Feature* feature, const FilterContext& context)
	{
		return compile(feature, *feature->style(), context);
	}

	osg::Node* PipelineGeometryCompiler::compile(FeatureCursor* cursor, const Style& style, const FilterContext& context)
	{
	...
	osg::Node* PipelineGeometryCompiler::compile(FeatureList& workingSet, const Style& style, const FilterContext& context)
	{
#ifdef PROFILING
		...
		const PipeLineSymbol     * gwps		     = style.get<PipeLineSymbol>();
		const PipeJointSymbol    * pipejoint     = style.get<PipeJointSymbol>();
		const PipeEquipmentSymbol* pipeequipment = style.get<PipeEquipmentSymbol>();

		if (gwps || pipejoint || pipeequipment)
		{
			BuildPipelineGeometryFilter filter( style );
