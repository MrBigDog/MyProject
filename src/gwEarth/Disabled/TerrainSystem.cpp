...
typedef void(gwEarth::TerrainSystem::*method_f)(const osgEarth::Config& arg);
...
namespace gwEarth
{
	...
	void TerrainSystem::addImageLayer(const osgEarth::Config & arg)
	{
		...
		(
			new LayerOperation(this, &TerrainSystem::addImageLayerMethod, arg)
		);
	}
	...
	void TerrainSystem::insertImageLayer(const osgEarth::Config & arg)
	{
		...
		_app->getRender()->addOperation
		(
			new LayerOperation(this, &TerrainSystem::insertImageLayerMethod, arg)
		);
	}
	...
	void TerrainSystem::removeImageLayer(const std::string& layerName)
	{
		...
		_app->getRender()->addOperation
		(
			new LayerOperation(this, &TerrainSystem::removeImageLayerMethod, arg)
		);
	}
	...
	void TerrainSystem::moveImageLayer(const std::string& layerName, int newIndex)
	{
		...
		osgEarth::Config arg;
		arg.add("layer_name", layerName);
		arg.add("new_index", newIndex);
		_app->getRender()->addOperation
		(
			new LayerOperation(this, &TerrainSystem::moveImageLayerMethod, arg)
		);
	}
	...
	//elevation
	void TerrainSystem::addElevationLayer(const osgEarth::Config&arg)
	{
		if (!_app || !_app->getMap() || !_app->getRender())
			return;
		...
		_app->getRender()->addOperation
		(
			new LayerOperation(this, &TerrainSystem::addElevationlayerMethod, arg)
		);
	}
	...
	void TerrainSystem::removeElevationLayer(const std::string& layerName)
	{
		...
		_app->getRender()->addOperation
		(
			new LayerOperation(this, &TerrainSystem::removeElevationLayerMethod, arg)
		);
	}
	...
	void TerrainSystem::moveElevationLayer(const std::string& layerName, int newIndex)
	{
		...
		_app->getRender()->addOperation
		(
			new LayerOperation(this, &TerrainSystem::moveElevationLayerMethod, arg)
		);
	}
	...
	{
		return _app->getMapNode()->getTerrainEngine();
	}
	...
	void TerrainSystem::addImageLayerMethod(const osgEarth::Config & arg)
	{
		...
		_app->getMap()->beginUpdate();
		_app->getMap()->addImageLayer(layer);
		_app->getMap()->endUpdate();
		TerrainSystemCallbackVector::const_iterator it;
		for (it = _callbacks.begin(); it != _callbacks.end(); ++it)
		{
			if (!(*it) || (*it) == NULL) continue;
			(*it)->onImageLayerAdded(layer);
		}
	}
	...
	void TerrainSystem::moveImageLayerMethod(const osgEarth::Config & arg)
	{
		...
		{
			if (!(*it) || (*it) == NULL) continue;
			(*it)->onImageLayerMoved(layer, newindex);
		}
	}
	...
	void TerrainSystem::insertImageLayerMethod(const osgEarth::Config & arg)
	{
		...
		_app->getMap()->beginUpdate();
		_app->getMap()->insertImageLayer(layer, index);
		_app->getMap()->endUpdate();
		...
		{
			if (!(*it) || (*it) == NULL) continue;
			(*it)->onImageLayerInserted(layer, index);
		}
	}
	...
	void TerrainSystem::addElevationlayerMethod(const osgEarth::Config & arg)
	{
		...
		_app->getMap()->beginUpdate();
		_app->getMap()->addElevationLayer(layer);
		_app->getMap()->endUpdate();
		TerrainSystemCallbackVector::const_iterator it;
		for (it = _callbacks.begin(); it != _callbacks.end(); ++it)
		{
			if (!(*it) || (*it) == NULL) continue;
			(*it)->onElevationLayerAdded(layer);
		}
	}
	...
	void TerrainSystem::moveElevationLayerMethod(const osgEarth::Config & arg)
	{
		...
		{
			if (!(*it) || (*it) == NULL) continue;
			(*it)->onElevationLayerMoved(layer, newindex);
		}
	}
	...
	void TerrainSystem::removeElevationLayerMethod(const osgEarth::Config & arg)
	{
		...
		{
			if (!(*it) || (*it) == NULL) continue;
			(*it)->onElevationLayerRemoved(layer);
		}
	}
