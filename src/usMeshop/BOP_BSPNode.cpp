								  ...
								  const MT_Plane3& plane )
{
	...
	if (tag == BOP_ON) { }		// face lies on hyperplane: do nothing
	else if ((tag & BOP_IN) != 0 && (tag & BOP_OUT) == 0) {	// face is entirely on inside
		if (m_inChild != NULL)
	...
	} else if ((tag & BOP_OUT) != 0 && (tag & BOP_IN) == 0) { // face is entirely on outside
		if (m_outChild != NULL)
		...
		for(BOP_IT_BSPPoints itp=pts.begin();itp!=ptsEnd;itp++){
			...
			if(ltag != BOP_ON) {	// last point not on hyperplane
				if(tstate == BOP_IN) {
					if (m_inChild != NULL) inside.push_back(lpoint);
				} else {
					if (m_outChild != NULL) outside.push_back(lpoint);
				}
				if(ntag != BOP_ON && ntag != tstate) {	// last, self in different half-spaces 
