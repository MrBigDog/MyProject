...
#	define EPSILON /*FLT_EPSILON*/1.192092896e-06
	...
	static osg::Vec3dArray* getReverseArray(const osg::Vec3dArray* va)
	{
		...
		//for (unsigned int i = va->size() - 1; i > -1; --i)
		//{
...
namespace
{
	...
	struct GetGeometryMaterial
	{
		...
		osg::Texture2D* texture() { return _texture.get(); }
	...
	for (minIt = minLoops.begin(); minIt != minLoops.end(); ++minIt)
	{
		...
		if (ggm.texture())
		{
			...
			if (ta)
			{
				...
				geom->getOrCreateStateSet()->setTextureAttributeAndModes(0, gm->_texture/*attr->_texture*/);
			}
		}
		root->addChild(geode);
	}
...
osg::Node* SliceOperator::sliceGeom(osg::Geometry* geom, const osg::Plane& sliceplane, const osg::Matrixd& mat, osg::Group* testGroup)
{
	...
	//osgDB::writeNodeFile(*profileNode, "dogt.osgb");
	...
	if (testGroup)
	{
		...
		{//junction
			...
			osg::ref_ptr<osg::Geode> pg = generatePoint(va, osg::Vec4(1, 0, 0, 1), 10.0f);
			mt->addChild(pg);
		}
	}
