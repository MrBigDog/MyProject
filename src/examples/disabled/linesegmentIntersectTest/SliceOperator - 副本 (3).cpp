...
osg::Node* SliceOperator::slice(osg::Group * geologicalModel, const osg::Plane& slicePlane, osg::Group* root)
{
	...
	MyGeometryVector::const_iterator it;
	for (it = geoms.begin(); it != geoms.end(); ++it)
	{
		...
		osg::ref_ptr<osg::Node> profilenode = this->sliceGeom(geom, splane, matrix, root);
		if (!profilenode) continue;

		osg::ref_ptr<osg::MatrixTransform> mt = getMatrixTransform(matrix);
		mt->addChild(profilenode);
	...
	//{
	//	osg::Node* child = geologicalModel->getChild(i);
	//	if (!child) continue;
	...
	//		osg::ref_ptr<osg::Node> profilenode = this->sliceGeom(geom, slicePlane, matrix, root);
	//		if (!profilenode) continue;
	...
	//	//GeometryVisitor gv(this, slicePlane, profileroot, root);
	//	//child->accept(gv);
	//}
