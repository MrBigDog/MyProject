...
struct DirectedLoop :public osg::Referenced
{
	...
	DirectedLoop() :_type(UNDEFINED), _va(0L), _attribuate(0L) { _vaVector.clear(); }
...
void SliceOperator::generateJunctions(const SliceTrailVector& vas, const osg::Vec3d& planeNormal, JunctionVector& outJoints)
{
	...
	for (SliceTrailVector::const_iterator it = vas.begin(); it != vas.end(); ++it)
	{
		...
		path->attribuate() = (*it)->_attribuate;
...
void SliceOperator::generateBooleanPaths(Path* path, bool isClosedPath, BooleanPathVector &bpvector)
{
	...
	for (unsigned int i = 0; i < path->junctions().size() - 1; ++i)
	{
		...
		bp0->isPartOfClosed() = path->isClosed();
		bp0->attribuate() = path->attribuate();

		osg::ref_ptr<BooleanPath> bp1 = new BooleanPath(va1);
		bp1->startJunction() = nextJunction;
		bp1->endJunction() = currentJunction;
		bp1->isPartOfClosed() = path->isClosed();
		bp1->attribuate() = path->attribuate();
...
LayerAttribuate* SliceOperator::getAttribuate(const BooleanPathVector& bpaths)
{
	if (bpaths.empty()) return 0L;

	osg::ref_ptr<LayerAttribuate> att = 0L;

	double vlength = DBL_MIN;
	for (BooleanPathVector::const_iterator it = bpaths.begin(); it != bpaths.end(); ++it)
	{
		if ((*it)->isPartOfClosed()) continue;

		osg::Vec3d c1;
		for (osg::Vec3Array::const_iterator vaIt = (*it)->pointArray()->begin(); vaIt != (*it)->pointArray()->end(); ++vaIt)
		{
			c1 += *vaIt;
		}
		c1 = c1 / (*it)->pointArray()->size();
