...
#define TEST_EPSILON 1.0//精度低些可以增强容错能力;
#define JUNCTION_EPSILON 0.01

#define PI2 6.28318530717958647692

#define MIN_JUNCTION_DIS 1.5
...
#	define EPSILON /*FLT_EPSILON*/1.192092896e-06
	...
	static osg::Vec3dArray* getSubArrayOfLoop(const osg::Vec3dArray* loopVa, int first, int last)
	{
		...
		if (first > last)
		{
			subva->reserve(maxIndex - first + last + 2);
	...
	static bool isSameArray(const osg::Vec3dArray* va1, const osg::Vec3dArray* va2)
	{
...
void SliceOperator::generateJunctions(Path* p1, Path* p2, JunctionVector& joints)
{
	...
	for (unsigned int i = 0; i < p1->_va->size(); ++i)
	{
		...
		for (unsigned int j = 0; j < p2->_va->size(); ++j)
		{
			...
			if (isVec3dEqual(v1, v2, JUNCTION_EPSILON))
			{
...
void SliceOperator::generateJunctions(const Vec3dArrayVector& vas, const osg::Vec3d& planeNormal, JunctionVector& outJoints)
{
	...
	////删除距离start junction过近的点，避免求顺时针夹角的时候出现误差;(不用删了)
	//for (BooleanPathVector::const_iterator it = bpvector.begin(); it != bpvector.end(); ++it)
	//{
	//	const osg::Vec3d& ps = (*it)->startJunction()->_position;
	//	osg::Vec3dArray::iterator vaIt;
	//	for (vaIt = (*it)->pointArray()->begin(); vaIt != (*it)->pointArray()->end(); )
	//	{
	//		if (isVec3dEqual(*vaIt, ps, TEST_EPSILON))
	//		{
	//			vaIt = (*it)->pointArray()->erase(vaIt);
	//		}
	//		else
	//		{
	//			vaIt++;
...
bool isJunctionPoint(const unsigned int& pointIndex, const Path::JunctionPairVector& junctions)
{
	for (Path::JunctionPairVector::const_iterator it = junctions.begin(); it != junctions.end(); ++it)
	{
...
void SliceOperator::generateBooleanPaths(Path* path, bool isClosedPath, BooleanPathVector &bpvector)
{
	...
	else
	{
		for (unsigned int i = 0; i < path->_junctions.size() - 1; ++i)
		{
			...
			{
				//忽略重合的;
				bool isAllreadyHas = false;
				for (BooleanPathVector::const_iterator it = bpvector.begin(); it != bpvector.end(); ++it)
				{
					if (isSameArray(va0, (*it)->pointArray()))
					{
						isAllreadyHas = true;
						break;
					}
