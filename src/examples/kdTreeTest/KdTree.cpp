...
{
	BuildKdTree builder(*this);
	return builder.build(options, geometry);
}
...
//
// KdTreeBuilder
KdTreeBuilder::KdTreeBuilder() : osg::NodeVisitor(osg::NodeVisitor::TRAVERSE_ALL_CHILDREN)
{
...
{
	KdTree* previous = dynamic_cast<KdTree*>(geometry.getShape());
	if (previous) return;

	osg::ref_ptr<KdTree> kdTree = osg::clone(_kdTreePrototype.get());
