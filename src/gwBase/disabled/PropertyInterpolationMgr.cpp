...
namespace simgear
{
	...
	void PropertyInterpolationMgr::update(double dt)
	{
		...
		for (InterpolatorList::iterator it = _interpolators.begin(); it != _interpolators.end(); ++it)
		{
			...
			const std::string& easing)
	{
		...
		{
			//SG_LOG(SG_GENERAL, SG_WARN,	"PropertyInterpolationMgr: no factory found for type '" << type << "'");
			return 0;
		}
		...
		{
			//SG_LOG ( SG_GENERAL, SG_WARN, "PropertyInterpolationMgr: no such easing '" << type << "'" );
			return 0;
		}
	...
	//----------------------------------------------------------------------------
	bool PropertyInterpolationMgr::interpolate(SGPropertyNode* prop, PropertyInterpolatorRef interp)
	{
		if (!prop)
		{
			return false;
		...
		InterpolatorList::iterator it = std::find_if(_interpolators.begin(), _interpolators.end(), PredicateIsSameProp(prop));
		if (!interp)
		{
	...
	void PropertyInterpolationMgr::addInterpolatorFactory(const std::string& type, InterpolatorFactory factory)
	{
		...
		{
			//SG_LOG(SG_GENERAL, SG_WARN, "PropertyInterpolationMgr: replace existing factor for type " << type);
		}
	...
	void PropertyInterpolationMgr::addEasingFunction(const std::string& type, easing_func_t func)
	{
		...
		{
			//SG_LOG(SG_GENERAL, SG_WARN, "PropertyInterpolationMgr: replace existing easing function " << type);
		}
