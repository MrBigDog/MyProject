#include <compiler.h>   // GW_WINDOWS
#include <gwBase/md5.h>
...
namespace gwBase
{
	namespace strutils
	{
		...
		static vector<string> split_whitespace(const string& str, int maxsplit)
		{
		...
		string_list split_on_any_of(const std::string& str, const char* seperators)
		{
			...
			{
				throw gw_exception("illegal/missing seperator string");
			}
		...
		static string do_strip(const string& s, int striptype)
		{
		...
		void stripTrailingNewlines_inplace(string& s)
		{
		...
		string stripTrailingNewlines(const string& s)
		{
			string res = s;
		...
		string rpad(const std::string & s, size_t length, char c)
		{
		...
		string lpad(const string & s, size_t length, char c)
		{
			string::size_type l = s.length();
			if (l >= length) return s;
			string reply = s;
		...
		bool starts_with(const string & s, const string & substr)
		{
			return s.compare(0, substr.length(), substr) == 0;
		}

		bool ends_with(const string & s, const string & substr)
		{
			...
			return s.compare(s.length() - substr.length(), substr.length(), substr) == 0;
		}
		...
		std::string md5(const unsigned char* data, size_t num)
		{
			GW_MD5_CTX md5_ctx;
			GW_MD5Init(&md5_ctx);
			GW_MD5Update(&md5_ctx, data, num);

			unsigned char digest[MD5_DIGEST_LENGTH];
			GW_MD5Final(digest, &md5_ctx);
		...
		string sanitizePrintfFormat(const string& input)
		{
			...
			if (i != string::npos)
			{
				GW_LOG(GW_IO, GW_WARN, "sanitizePrintfFormat: bad format string:" << input);
				return string();
			}
...
#elif (_POSIX_C_SOURCE >= 200112L) || defined(GW_MAC) || defined(__FreeBSD__)
			int retcode;
